...
#            define png_convert_to_rfc1123_buffer(ts, t)                       \
                tIME_to_str(read_ptr, ts, t)
...
static int set_location(png_structp png_ptr, struct user_chunk_data *data,
                        int what)
{
    int location;
...
#        if defined(PNG_USER_MEM_SUPPORTED) && PNG_DEBUG
    write_ptr =
        png_create_write_struct_2(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL, NULL,
...
#    ifdef PNG_SET_UNKNOWN_CHUNKS_SUPPORTED
     ...
     * save option on!)
     *
     * Notice that if SET_UNKNOWN_CHUNKS is *not* supported read will discard
     * all unknown chunks and write will write them all.
     */
        ...
        if (png_get_eXIf_1(read_ptr, read_info_ptr, &exif_length, &exif) != 0)
        {
            if (exif_length > 1)
                fprintf(STDERR, " eXIf type %c%c, %lu bytes\n", exif[0],
                        exif[1], (unsigned long)exif_length);
...
#        if defined(PNG_FLOATING_POINT_SUPPORTED)                              \
            && defined(PNG_FLOATING_ARITHMETIC_SUPPORTED)
    {
        ...
        if (png_get_sCAL(read_ptr, read_info_ptr, &unit, &scal_width,
            ...
            != 0)
        {
            png_set_sCAL(write_ptr, write_info_ptr, unit, scal_width,
                         scal_height);
        }
    }
#        else
...
#        ifdef PNG_TIME_RFC1123_SUPPORTED
            ...
            {
                strncpy(tIME_string, "*** invalid time ***",
                        (sizeof tIME_string));
                tIME_string[(sizeof tIME_string) - 1] = '\0';
            }
...
#        if PNG_LIBPNG_VER < 10600
             ...
             * generated locations in write_end_info_ptr are wrong prior
             * to 1.6.0 because they are reset from the write pointer (removed
             * in 1.6.0).
             */
            {
                int i;
...
#    ifndef SINGLE_ROWBUF_ALLOC
            pngtest_debug2("Allocating row buffer (pass %d, y = %u)...", pass,
                           y);
             ...
             */
            {
                ...
                for (i = 0; i < num_unknowns; i++)
                    png_set_unknown_chunk_location(
                        write_ptr, write_end_info_ptr, i, unknowns[i].location);
            }
#        endif
        }
    }
...
#    endif
    ...
    {
        fprintf(STDERR, "\n  %s: %d libpng warnings found", inname,
                warning_count);
        ...
        for (;;)
        {
            ...
            if (num_in != num_out)
            {
                ...
                if (wrote_question == 0 && unsupported_chunks == 0)
                {
                    ...
                    fprintf(STDERR,
                            "\n   filtering heuristic (libpng default), "
                            "compression");
                    fprintf(
                        STDERR,
                        " level (zlib default),\n   and zlib version (%s)?\n\n",
            ...
            if (memcmp(inbuf, outbuf, num_in))
            {
                 ...
                 * unsupported text chunk compression will result in the
                 * compression mode being changed (to NONE) yet, in the test
                 * case, the result can be exactly the same size!
                 */
                if (strict != 0 && unsupported_chunks == 0)
                    return (1);
...
#    endif
    ...
    if ((multiple == 0 && argc > 3 + verbose) || (multiple != 0 && argc < 2))
    {
        fprintf(STDERR,
                ...
                argv[0], argv[0]);
        fprintf(STDERR,
                "  reads/writes one PNG file (without -m) or multiple files "
                "(-m)\n");
