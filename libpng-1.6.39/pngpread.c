...
size_t PNGAPI png_process_data_pause(png_structrp png_ptr, int save)
{
    if (png_ptr != NULL)
    {
        /* It's easiest for the caller if we do the save; then the caller
         * doesn't have to supply the same data again:
         */
        if (save != 0)
            png_push_save_buffer(png_ptr);
        else
...
png_uint_32 PNGAPI png_process_data_skip(png_structrp png_ptr)
{
    ...
    png_app_warning(png_ptr,
                    "png_process_data_skip is not implemented in any current "
                    "version of libpng");
    return 0;
}
...
png_push_save_buffer(png_structrp png_ptr)
{
        ...
        > png_ptr->save_buffer_max)
    {
        ...
        png_ptr->save_buffer =
            (png_bytep)png_malloc_warn(png_ptr, (size_t)new_max);
...
png_push_read_IDAT(png_structrp png_ptr)
{
    ...
    if (png_ptr->idat_size != 0 && png_ptr->save_buffer_size != 0)
    {
        ...
        /* We want the smaller of 'idat_size' and 'current_buffer_size', but
         * they are of different types and we don't know which variable has the
         * fewest bits.  Carefully select the smaller and cast it to the type of
         * the larger - this cannot overflow.  Do not cast in the following test
         * - it will break on either 16-bit or 64-bit platforms.
         */
