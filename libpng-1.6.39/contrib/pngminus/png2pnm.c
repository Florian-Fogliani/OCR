        ...
        if (argv[argi][0] == '-')
        {
            ...
            case 'a':
                ...
                if ((fp_al = fopen(argv[argi], "wb")) == NULL)
                {
                    ...
                    fprintf(stderr,
                            "Error:  cannot create alpha-channel file %s\n",
                            argv[argi]);
                    exit(1);
                }
                break;
             ...
             BOOL alpha)
{
    ...
    if ((png_pixels = (png_byte *)malloc((size_t)row_bytes * (size_t)height))
        == NULL)
    {
        png_destroy_read_struct(&png_ptr, &info_ptr, NULL);
        return FALSE;
    ...
    if ((row_pointers =
             (png_byte **)malloc((size_t)height * sizeof(png_byte *)))
        == NULL)
    {
        png_destroy_read_struct(&png_ptr, &info_ptr, NULL);
        free(png_pixels);
