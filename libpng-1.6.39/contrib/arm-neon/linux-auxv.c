                        ...
                        size_t nbytes)
{
    ...
    while (nbytes > 0)
    {
        ...
        if (iread == -1)
        {
            /* This is the devil in the details, a read can terminate early with
             * 0 bytes read because of EINTR, yet it still returns -1 otherwise
             * end of file cannot be distinguished.
             */
            if (errno != EINTR)
            {
