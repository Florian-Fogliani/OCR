...

    {
...

        {
            ...
            for (pass = 0; pass < passes; ++pass)
            {
                ...
                /* NOTE: this trashes the row each time; interlace handling
                 * won't work, but this avoids memory thrashing for speed
                 * testing and is somewhat representative of an application that
                 * works row-by-row.
                 */
                while (y-- > 0)
...
static int add_one_file(FILE *fp, char *name)
{
    ...
    if (ip != NULL)
    {
         ...
         * file.  This must happen after the ferror check above to avoid
         * clearing the error.
         */
        if (fsetpos(fp, &pos))
        {
...
static void usage(FILE *fp)
{
    ...
    fprintf(stderr,
            "{files}:\n"
            "  PNG files to copy into the assembly and time.  Invalid files "
            "are skipped\n"
            "  with appropriate error messages.  If no files are given the "
            "list of files\n"
