        ...
        else
        {
            ...
            for (x = 0; x < size; ++x)
            {
                for (y = 0; y < size; ++y)
                {
                    ...
                    /* size is at most 16, so the scaled value below fits in 16
                     * bits
                     */
             ...
             unsigned int *colors, int small)
{
    ...
    if (colors[0] == 0 && small)
    {
        ...
        /* For pixel depths less than 16 generate a single row containing all
         * the possible pixel values.  For 16 generate all 65536 byte pair
        ...
        {
            /* 1 channel: a square image with a diamond, the least luminous
             * colors are on the edge of the image, the most luminous in the
             * center.
             */
        case 1: {
            png_uint_32 x;
        ...
        break;
             ...
             * bottom, the alpha channel increases in opacity from left to
             * right.
             */
        case 2: {
            ...
            for (x = 0; x <= size_max; ++x)
            {
                set_value(row, rowbytes, 2 * x, bit_depth,
                          ...
                          gamma_table, conv);
                set_value(row, rowbytes, 2 * x + 1, bit_depth, alpha,
                          gamma_table, conv);
            }
        }
        break;

            /* 3 channels: linear combinations of, from the top-left corner
             * clockwise, black, green, white, red.
             */
        case 3: {
            ...
            for (x = 0; x <= size_max; ++x)
            {
                ...
                set_value(row, rowbytes, 3 * x + 2, bit_depth, /* blue */
                          (Y * x * 2 + size_max) / (2 * size_max), gamma_table,
                          conv);
            }
        }
        break;

            /* 4 channels: linear combinations of, from the top-left corner
             * clockwise, transparent, red, green, blue.
             */
        case 4: {
            ...
            for (x = 0; x <= size_max; ++x)
            {
                ...
                set_value(row, rowbytes, 4 * x + 2, bit_depth, /* blue */
                          ...
                          gamma_table, conv);
                set_value(
                    row, rowbytes, 4 * x + 3, bit_depth, /* alpha */
    ...
    {
        /* Palette with fixed color: the image rows are all 0 and the image
         * width is 16.
         */
        memset(row, 0, rowbytes);
    }

    else if (colors[0] == channels_of_type(color_type)) switch (
        channels_of_type(color_type))
    {
    case 1: {
          ...
          unsigned int filters, unsigned int *colors, int small, int tRNS)
{
    png_structp png_ptr = png_create_write_struct(
        PNG_LIBPNG_VER_STRING, name, makepng_error, makepng_warning);
...

    {
        ...
        /* Normally images are square, but with 'small' we want to simply
         * generate all the pixel values, or all that we reasonably can:
         */
        if (small)
        {
        ...
        while (insert != NULL)
        {
            insert->insert(png_ptr, info_ptr, insert->nparams,
                           insert->parameters);
            insert = insert->next;
        }
        ...
        if (ip != NULL)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (total > 0)
                    {
                        /* Round up to a multiple of 4 here to allow an iCCP
                         * profile to be padded to a 4x boundary.
                         */
                        ...
                        png_charpp params)
{
    ...
    if (proflen & 3)
    {
        ...
        /* load_file allocates extra space for this padding, the ICC spec
         * requires padding with zero bytes.
         */
        while (proflen & 3)
            profile[proflen++] = 0;
    }
...
int main(int argc, char **argv)
{
    ...
    {
        unsigned int lim =
            (color_type == PNG_COLOR_TYPE_PALETTE ? 255U
                                                  : (1U << bit_depth) - 1);
        unsigned int i;
    ...
    /* small and colors are incompatible (will probably crash if both are used
     * at the same time!)
     */
    if (small && colors[0] != 0)
    {
