...
#        define PNG_PASS_ROWS(height, pass)                                    \
            (((height)                                                         \
              + (((1 << PNG_PASS_ROW_SHIFT(pass)) - 1)                         \
                 - PNG_PASS_START_ROW(pass)))                                  \
             >> PNG_PASS_ROW_SHIFT(pass))
#        define PNG_PASS_COLS(width, pass)                                     \
            (((width)                                                          \
              + (((1 << PNG_PASS_COL_SHIFT(pass)) - 1)                         \
 ...
 */
#    define FILEID(col, depth, palette, interlace, width, height,              \
                   do_interlace)                                               \
                                    ...
                                    png_uint_32 id)
{
    return standard_name(buffer, bufsize, pos, COL_FROM_ID(id),
                         DEPTH_FROM_ID(id), PALETTE_FROM_ID(id),
                         INTERLACE_FROM_ID(id), WIDTH_FROM_ID(id),
                         HEIGHT_FROM_ID(id), DO_INTERLACE_FROM_ID(id));
}
...
{
    /* The warning is copied to the error buffer if there are no errors and it
     * is the first warning.  The error is copied to the error buffer if it is
     * the first error (overwriting any prior warnings).
     */
                               ...
                               size_t cbRow, png_uint_32 cRows)
{
     ...
     * alignment starts out odd - 2+1 and changes for larger images on each
     * row.)
     */
    ps->cb_row = cbRow;
    ps->image_h = cRows;
...
static void PNGCBAPI store_write(png_structp ppIn, png_bytep pb, size_t st)
{
    ...
    /* Now process the bytes to be written.  Do this in units of the space in
     * the output (write) buffer or, at the start 4 bytes for the chunk type and
        ...
        do
        {
            ...
            if (chunktype == CHUNK_IDAT)
            {
                ...
                if (IDAT_pos == IDAT_len)
                {
                    ...
                    /* Make a new IDAT chunk, if IDAT_len is 0 this is the first
                     * IDAT, if IDAT_size is 0 this is the end.  At present this
                     * is set up using a random number so that there is a 25%
                     * chance before the start of the first IDAT chunk being 0
                     * length.
                     */
                    ...
                    {
                        /* The last (IDAT) chunk should be positioned at the CRC
                         * now: */
                        if (chunkpos != chunklen - 4U)
                            png_error(ps->pread,
                                      "internal: IDAT size mismatch");

                        /* The only option here is to add a zero length IDAT,
                         * this happens 25% of the time.  Because of the check
                         * above chunklen-4U-chunkpos must be zero, we just need
                         * to skip the CRC now.
                         */
                        if ((random & 3U) == 0U)
                        ...
                        {
                            /* End of IDATs, skip the CRC to make the code above
                             * load the next chunk header next time round.
                             */
                            png_byte buffer[4];
                    ...
                    {
                        /* Middle of IDATs, use 'random' to determine the number
                         * of bits to use in the IDAT length.
                         */
                        IDAT_len = random_u32();
                ...
                else if (IDAT_pos < IDAT_len - 4U) /* I.e not the CRC */
                {
                    if (chunkpos < chunklen - 4U)
                    {
                        ...
                        if (avail > (IDAT_len - 4U) - IDAT_pos)
                            avail =
                                (uInt) /*SAFE*/ ((IDAT_len - 4U) - IDAT_pos);
                        ...
                        if (avail > (chunklen - 4U) - chunkpos)
                            avail =
                                (uInt) /*SAFE*/ ((chunklen - 4U) - chunkpos);
            ...
            {
                /* If there is still some pending IDAT data after the IDAT
                 * chunks have been processed there is a problem:
                 */
                ...
                else /* Return chunk bytes, including the CRC */
                {
                    ...
                    /* Check for end of chunk and end-of-file; don't try to read
                     * a new chunk header at this point unless instructed to do
                     * so by 'min'.
                     */
...
static store_palette_entry *store_write_palette(png_store *ps, int npalette)
{
    ...
    if (npalette > 0)
    {
        ps->palette = voidcast(store_palette_entry *,
                               malloc(npalette * sizeof *ps->palette));
                              ...
                              store_memory *memory)
{
     ...
     * found 'memory' in pool->list *and* unlinked this entry, so this is a
     * valid pointer (for sure), but the contents may have been trashed.
     */
    if (memory->pool != pool)
...
static png_voidp PNGCBAPI store_malloc(png_structp ppIn, png_alloc_size_t cb)
{
    ...
    {
        /* NOTE: the PNG user malloc function cannot use the png_ptr it is
         * passed other than to retrieve the allocation pointer!  libpng calls
         * the store_malloc callback in two basic cases:
         *
...
#        ifdef PNG_USER_MEM_SUPPORTED
    if (!ps->speed)
        ps->pread = png_create_read_struct_2(
            PNG_LIBPNG_VER_STRING, ps, store_error, store_warning,
...
{
    const double whiteY =
        encoding->red.Y + encoding->green.Y + encoding->blue.Y;
...
typedef struct png_modifier
{
    ...
    /* Error control - these are the limits on errors accepted by the gamma
     * tests below.
     */
    ...
    /* If set assume that the calculations are done in 16 bits even if the
     * sample depth is 8 bits.
     */
    ...
    /* Buffer information, the buffer size limits the size of the chunks that
     * can be modified - they must fit (including header and CRC) into the
     * buffer!
     */
...
static double digitize(double value, int depth, int do_round)
{
    ...
    /* Limiting the range is done as a convenience to the caller - it's easier
     * to do it once here than every time at the call site.
     */
    if (value <= 0)
        value = 0;
...
{
    /* The command line parameters are either 8 bit (0..255) or 16 bit
     * (0..65535) and so must be adjusted for low bit depth grayscale:
     */
    if (out_depth <= 8)
    {
...
typedef struct png_modification
{
     ...
     * found and modified (and there is a modify_fn) the modify_fn will be
     * called to add the chunk before the relevant chunk.
     */
    png_uint_32 add;
...
static void modifier_encoding_iterate(png_modifier *pm)
{
        ...
        pm->test_uses_encoding) /* Some transform is encoding dependent */
    {
        ...
        {
            /* Not exhaustive - choose an encoding at random; generate a number
             * in the range 1..(max-1), so the result is always non-zero:
             */
            if (pm->encoding_counter == 0)
                pm->encoding_counter =
                    random_mod(modifier_total_encodings(pm) - 1) + 1;
            else
                pm->encoding_counter = 0;
        }
...
static void modifier_set_encoding(png_modifier *pm)
{
     ...
     * first clear out all the settings - this corresponds to an
     * encoding_counter of 0.
     */
    pm->current_gamma = 0;
    pm->current_encoding = 0;
    ...
    while (st > 0)
    {
        ...
        if (pm->buffer_position >= pm->buffer_count)
            ...
            case modifier_signature:
                store_read_chunk(&pm->this, pm->buffer, 13 + 12,
                                 13 + 12); /* IHDR */
            ...
            default:
                    ...
                    || chunk == CHUNK_IEND)
                {
                    ...
                    while (mod != NULL)
                    {
                        if ((mod->add == chunk
                            ...
                            && mod->modify_fn != NULL && !mod->modified
                            && !mod->added)
                        {
                            /* Regardless of what the modify function does do
                             * not run this again.
                             */
                            mod->added = 1;
                            ...
                            if ((*mod->modify_fn)(pm, mod, 1 /*add*/))
                            {
                                ...
                                /* The buffer has been filled with something (we
                                 * assume) so output this.  Pend the current
                                 * chunk.
                                 */
                                pm->pending_len = len;
                    ...
                    /* Don't do any further processing if the buffer was
                     * modified - otherwise the code will end up modifying a
                     * chunk that was just added.
                     */
                    if (mod != NULL)
                        break; /* out of switch */
                }
                 ...
                 * do this it must be less than 1024 bytes in total size,
                 * otherwise it just gets flushed.
                 */
                if (len + 12 <= sizeof pm->buffer)
                {
                    size_t s = len + 12 - pm->buffer_count;
                    store_read_chunk(&pm->this, pm->buffer + pm->buffer_count,
                                     s, s);
                                      ...
                                      png_infop pi)
{
    ...
    for (;;)
    {
        ...
        /* Check that this number of bytes are available (in the current
         * buffer.) (This doesn't quite work - the modifier might delete a
         * chunk; unlikely but possible, it doesn't happen at present because
         * the modifier only adds chunks to standard images.)
         */
                                                  ...
                                                  int do_tRNS)
{
     ...
     */
    {
         ...
         * permutes the npalette == 256 case in a potentially useful way (there
         * is no relationship between palette index and the color value
         * therein!)
         */
                                 ...
                                 int interlace_type, png_const_charp name)
{
    ...
    Try
    {
         ...
         * to do the clean up - it is not possible to 'return' directly from a
         * Try block.
         */
        if (pp == NULL)
            Throw ps;
...
#        endif
        {
            ...
            /* Use a compressed text string to test the correct interaction of
             * text compression and IDAT compression.
             */
...
#    endif
        ...
        else
        {
             ...
             * because if it is called before, the information in *pp has not
             * been updated to reflect the interlaced image.
             */
            ...
            for (pass = 0; pass < npasses; ++pass)
            {
                ...
                /* do_own_interlace is a pre-defined boolean (a #define) which
                 * is set if we have to work out the interlaced rows here.
                 */
                for (y = 0; y < h; ++y)
                {
                    png_byte buffer[TRANSFORM_ROWMAX];
...
#    if do_own_interlace
                    /* If do_own_interlace *and* the image is interlaced we need
                     * a reduced interlace row; this may be reduced to empty.
                     */
...
#    endif
    ...
    Catch(fault)
    {
        /* Use the png_store returned by the exception. This may help the
         * compiler because 'ps' is not used in this branch of the setjmp.  Note
         * that fault and ps will always be the same value.
         */
    ...
    while (next_format(&colour_type, &bit_depth, &palette_number, 1, 1))
    {
             ...
             interlace_type < INTERLACE_LAST; ++interlace_type)
        {
            ...
            standard_name(name, sizeof name, 0, colour_type, bit_depth,
                          palette_number, interlace_type, 0, 0,
                          do_own_interlace);
...
#    endif
        ...
        else
        {
            ...
            for (pass = 0; pass < npasses; ++pass)
            {
                 ...
                 * row because some of them are empty.  In fact, for a 1x1
                 * image, most of them are empty!
                 */
                for (y = 0; y < h; ++y)
                {
                    ...
                    if (do_interlace && interlace_type == PNG_INTERLACE_ADAM7)
                    {
                        ...
                        {
                            /* Set to all 1's for error detection (libpng tends
                             * to set unset things to 0).
                             */
...
#    ifdef PNG_WRITE_FILTER_SUPPORTED
                    /* Only get to here if the row has some pixels in it, set
                     * the filters to 'all' for the very first row and
                     * thereafter to a single filter.  It isn't well documented,
                     * but png_set_filter does accept a filter number (per the
                     * spec) as well as a bit mask.
                     *
                     * The code now uses filters at random, except that on the
                     * first row of an image it ensures that a previous row
                     * filter is in the set so that libpng allocates the row
                     * buffer.
                     */
                    {
    ...
    for (; bdlo <= bdhi; ++bdlo)
    {
        ...
        for (width = 1; width <= 16; ++width)
        {
            ...
            for (height = 1; height <= 16; ++height)
            {
                 ...
                 * libpng doing it.
                 */
                make_size_image(ps, colour_type, DEPTH(bdlo),
                                PNG_INTERLACE_NONE, width, height, 0);
                make_size_image(ps, colour_type, DEPTH(bdlo),
                                PNG_INTERLACE_NONE, width, height, 1);
#    ifdef PNG_WRITE_INTERLACING_SUPPORTED
                make_size_image(ps, colour_type, DEPTH(bdlo),
                                PNG_INTERLACE_ADAM7, width, height, 0);
#    endif
#    if CAN_WRITE_INTERLACE
                /* 1.7.0 removes the hack that prevented app write of an
                 * interlaced image if WRITE_INTERLACE was not supported
                 */
                make_size_image(ps, colour_type, DEPTH(bdlo),
                                PNG_INTERLACE_ADAM7, width, height, 1);
#    endif
            }
        }
    }
...
#        undef exception__env
        ...
        {
            /* Now write the whole image, just to make sure that the detected,
             * or undetected, error has not created problems inside libpng. This
             * doesn't work if there was a png_error in png_write_info because
             * that can abort before PLTE was written.
             */
            if (png_get_rowbytes(pp, pi)
                != transform_rowsize(pp, colour_type, bit_depth))
...
#        if do_own_interlace
                        ...
                        {
                            /* The row must not be written if it doesn't exist,
                             * notice that there are two conditions here, either
                             * the row isn't ever in the pass or the row would
                             * be but isn't wide enough to contribute any
                             * pixels.  In fact the wPass test can be used to
                             * skip the whole y loop in this case.
                             */
                            if (PNG_ROW_IN_INTERLACE_PASS(y, pass)
                                && PNG_PASS_COLS(w, pass) > 0)
                                interlace_row(
                                    buffer, buffer,
                                    bit_size(pp, colour_type, bit_depth), w,
                                    pass, 0 /*data always bigendian*/);
                            else
                                continue;
                        }
#        endif /* do_own_interlace */
...
#    ifdef PNG_TIME_RFC1123_SUPPORTED
    /* The handle into the formatting code is the RFC1123 support; this test
     * does nothing if that is compiled out.
     */
    context(ps, fault);
                        ...
                        png_const_structp pp, png_infop pi)
{
    ...
    *npalette = -1;
    ...
    if ((png_get_tRNS(pp, pi, &trans_alpha, &num, 0) & PNG_INFO_tRNS) != 0
        && (trans_alpha != NULL
            || num != 1 /*returns 1 for a transparent color*/)
        &&
        /* Oops, if a palette tRNS gets expanded png_read_update_info (at least
         * so far as 1.5.4) does not remove the trans_alpha pointer, only
         * num_trans, so in the above call we get a success, we get a pointer
         * (who knows what to) and we get num_trans == 0:
         */
                                ...
                                png_infop pi)
{
    ...
    /* Record (but don't check at present) the input sBIT according to the
     ...
     * no transforms, but it does for other tests where rowbytes may change
     * after png_read_update_info.
     */
     ...
     */
    {
        ...
        if (png_get_tRNS(pp, pi, 0, 0, &trans_color) & PNG_INFO_tRNS)
        {
            ...
            {
            case 0:
                dp->transparent.red = dp->transparent.green =
                    dp->transparent.blue = trans_color->gray;
                dp->has_tRNS = 1;
                break;
                              ...
                              png_infop pi, int nImages)
{
    ...
    /* Validate the height, width and rowbytes plus ensure that sufficient
     * buffer exists for decoding the image.
     */
    ...
    for (pass = 0; pass < npasses; ++pass)
    {
        ...
        for (y = 0; y < height; ++y)
        {
            if (do_interlace)
            {
                ...
                {
                    /* Read the row into a pair of temporary buffers, then do
                     * the merge here into the output rows.
                     */
                    png_byte row[STANDARD_ROWMAX], display[STANDARD_ROWMAX];

                    /* The following aids (to some extent) error detection - we
                     * can see where png_read_row wrote.  Use opposite values in
                     * row and display to make this easier.  Don't use 0xff
                     * (which is used in the image write code to fill unused
                     * bits) or 0 (which is a likely value to overwrite unused
                     * bits with).
                     */
                    ...
                    if (iDisplay >= 0)
                        deinterlace_row(store_image_row(ps, pp, iDisplay, y),
                                        display, dp->pixel_size, dp->w, pass,
                                        dp->littleendian);
                }
            }
            else
                png_read_row(
                    ...
                    iDisplay >= 0 ? store_image_row(ps, pp, iDisplay, y)
                                  : NULL);
        }
    }
                          ...
                          int do_interlace, int use_update_info)
{
    ...
    Try
    {
        ...
        /* Get a png_struct for reading the image. This will throw an error if
         * it fails, so we don't need to check the result.
         */
        pp = set_store_for_read(
            ...
            d.do_interlace
                ...
                : (d.ps->progressive ? "progressive reader"
                                     : "sequential reader"));
        ...
        else
        {
            ...
            /* Need the total bytes in the image below; we can't get to this
             * point unless the PNG file values have been checked against the
             * expected values.
             */
            {
                sequential_row(&d, pp, pi, 0, 1);

                /* After the last pass loop over the rows again to check that
    ...
    for (; bdlo <= bdhi; ++bdlo)
    {
        ...
        for (h = 1; h <= 16; h += hinc[bdlo])
        {
            for (w = 1; w <= 16; w += winc[bdlo])
            {
                 ...
                 * validates the write side of libpng.  There are four
                 * possibilities to validate.
                 */
                standard_test(&pm->this,
                              FILEID(colour_type, DEPTH(bdlo), 0 /*palette*/,
...
#        ifdef PNG_READ_INTERLACING_SUPPORTED
                 ...
                 * already verify these are ok (unless pngvalid has
                 * self-consistent read/write errors, which is unlikely), so
                 * this detects errors in the read side first:
                 */
#            if CAN_WRITE_INTERLACE
                standard_test(&pm->this,
                              FILEID(colour_type, DEPTH(bdlo), 0 /*palette*/,
...
typedef struct image_pixel
{
     ...
     * channel is stored.  This sBIT value is folded in to the stored error
     * value at the end of the application of the transforms to the pixel.
     *
                 ...
                 const image_pixel *format /*from pngvalid transform of input*/)
{
     ...
     * Note that this currently assumes a simple palette where entry x has
     * colour rgb(x,x,x)!
     */
...
static void image_pixel_convert_PLTE(image_pixel *this)
{
    if (this->colour_type == PNG_COLOR_TYPE_PALETTE)
    {
        ...
        /* The bit depth of the row changes at this point too (notice that this
         * is the row format, not the sample depth, which is separate.)
         */
        this->bit_depth = 8;
    }
}
...
#                endif
            ...
            if (this->have_tRNS)
            {
                /* After 1.7 the expansion of bit depth only happens if there is
                 * a tRNS chunk to expand at this point.
                 */
#                if PNG_LIBPNG_VER >= 10700
                if (!for_background && this->bit_depth < 8)
                    this->bit_depth = this->sample_depth = 8;
#                endif
        ...
        else if (this->colour_type == PNG_COLOR_TYPE_RGB)
        {
            if (this->have_tRNS)
            {
                ...
                /* Again, check the exact input values, not the current
                 * transformed value!
                 */
                if (this->red == display->transparent.red
         ...
         * original sBIT data (actually it will always be the original bit
         * depth).
         */
        this->alphae = 0;
...
static void transform_set_encoding(transform_display *this)
{
    ...
    if (modifier_color_encoding_is_set(pm))
    {
        if (modifier_color_encoding_is_sRGB(pm))
            srgb_modification_init(&this->srgb_mod, pm,
        ...
        else
        {
            ...
            if (pm->current_encoding != 0)
                chrm_modification_init(&this->chrm_mod, pm,
                                       pm->current_encoding);
        }
    }
}
                                    ...
                                    const transform_display *display)
{
     ...
     * the sBIT allows the implementation to be worse than this.  In addition
     * the PNG specification actually permits any error within the range
     * (-1..+1), but that is ignored here.  Instead the final digitized value is
     * compared, below to the digitized value of the error limits - this has the
     * net effect of allowing (almost) +/-1 in the output value.  It's difficult
     * to see how any algorithm that digitizes intermediate results can be more
     * accurate.
     */
                               ...
                               png_infop pi)
{
     ...
     * this avoids running the whole test if the output is unexpected.  This
     * also checks for internal errors.
     */
    {
        ...
        if (test_pixel.colour_type != dp->output_colour_type)
        {
            ...
            pos =
                safecatn(message, sizeof message, pos, dp->output_colour_type);
            pos = safecat(message, sizeof message, pos, " expected ");
            pos =
                safecatn(message, sizeof message, pos, test_pixel.colour_type);
                 ...
                 && test_pixel.bit_depth != test_pixel.sample_depth)
        {
            ...
            /* Because unless something has set 'unpacked' or the image is
             * palette mapped we expect the transform to keep sample depth and
             * bit depth the same.
             */
            pos =
                safecatn(message, sizeof message, pos, test_pixel.sample_depth);
            pos = safecat(message, sizeof message, pos, " expected ");
            pos = safecatn(message, sizeof message, pos, test_pixel.bit_depth);
        ...
        else if (test_pixel.bit_depth != dp->output_bit_depth)
        {
            ...
            char message[128];
            size_t pos =
                safecat(message, sizeof message, 0, "internal: bit depth ");
...
{
    transform_info_imp(
        voidcast(transform_display *, png_get_progressive_ptr(pp)), pp, pi);
}
                                  ...
                                  double digitization_error)
{
    ...
    if (debugonly(err > limit ||) !(out >= in_min && out <= in_max))
    {
        ...
        pos =
            safecat(message, sizeof message, pos, " output value error: rgba(");
                                     ...
                                     png_const_structp pp, png_infop pi)
{
    ...
    else
    {
        ...
        /* use-input-precision means assume that if the input has 8 bit (or
         * less) samples and the output has 16 bit samples the calculations will
         * be done with 8 bit precision, not 16.
         */
         ...
         * consists of scaled 8-bit values, so scale .5 in 8 bits to the 16
         * bits:
         */
        else
    ...
    for (y = 0; y < h; ++y)
    {
        ...
        /* Go through each original pixel transforming it and comparing with
         * what libpng did to the same pixel.
         */
        for (x = 0; x < w; ++x)
        {
            ...
            /* We don't expect changes to the index here even if the bit depth
             * is changed.
             */
            ...
            /* Check the colours for palette images too - in fact the palette
             * could be separately verified itself in most cases.
             */
            if (in_pixel.red != out_pixel.red)
                transform_range_check(
                    pp, r, g, b, a, in_pixel.red, in_pixel.redf, out_pixel.red,
                    sample_depth, in_pixel.rede,
            ...
            if ((out_ct & PNG_COLOR_MASK_COLOR) != 0
                ...
                transform_range_check(
                    pp, r, g, b, a, in_pixel.green, in_pixel.greenf,
            ...
            if ((out_ct & PNG_COLOR_MASK_COLOR) != 0
                ...
                transform_range_check(
                    pp, r, g, b, a, in_pixel.blue, in_pixel.bluef,
            ...
            if ((out_ct & PNG_COLOR_MASK_ALPHA) != 0
                ...
                transform_range_check(
                    pp, r, g, b, a, in_pixel.alpha, in_pixel.alphaf,
...
#            define IT(name)                                                   \
                static image_transform ITSTRUCT(                               \
                    name) = { #name,                                           \
                              1, /*enable*/                                    \
                              &PT, /*list*/                                    \
                              0, /*global_use*/                                \
                                        ...
                                        const transform_display *display)
{
    ...
    if (that->colour_type == PNG_COLOR_TYPE_GRAY)
    {
        ...
        /* And just changing the colour type works here because the green and
         * blue channels are being maintained in lock-step with the red/gray:
         */
...
static void image_transform_png_set_expand_mod(const image_transform *this,
                                               image_pixel *that,
 ...
 */
static void
image_transform_png_set_expand_gray_1_2_4_to_8_set(const image_transform *this,
                                                   transform_display *that,
                                                   png_structp pp, png_infop pi)
{
                                     ...
                                     const transform_display *display)
{
    if (that->bit_depth == 16)
    {
        ...
        if (that->alpha_sBIT > 8)
            ...
            /* Prior to 1.5.4 png_set_strip_16 would use an 'accurate' method if
             * this configuration option is set.  From 1.5.4 the flag is never
             * set and the 'scale' API (above) must be used.
             */
                                                    ...
                                                    transform_display *that)
{
    ...
    if (e != 0)
    {
        /* Coefficients come from the encoding, but may need to be normalized to
         * a white point Y of 1.0
         */
...
#                endif
    ...
    /* The arguments to png_set_rgb_to_gray can override the coefficients
     * implied by the color space encoding.  If doing exhaustive checks do the
     * override in each case, otherwise do it randomly.
     */
    if (pm->test_exhaustive)
    {
        /* First time in coefficients_overridden is 0, the following sets it to
         * 1, so repeat if it is set.  If a test fails this may mean we
         * subsequently skip a non-override test, ignore that.
         */
...
#                endif
        ...
        {
            /* Rounding to 8 bits in the linear space causes massive errors
             * which will trigger the error check in transform_range_check.  Fix
             * that here by taking the gamma encoding into account.
             *
             * When DIGITIZE is set because a pre-1.7 version of libpng is being
             * tested allow a bigger slack.
             *
             * NOTE: this number only affects the internal limit check in
             * pngvalid, it has no effect on the limits applied to the libpng
             * values.
             */
#                if DIGITIZE
...
#                endif
    ...
    {
        /* With no gamma correction a large error comes from the truncation of
         * the calculation in the 8 bit case, allow for that here.
         */
        if (that->this.bit_depth != 16 && !pm->assume_16_bit_calculations)
            that->pm->limit += 4E-3;
    }
...
#                if DIGITIZE
        {
            ...
            /* Do this using interval arithmetic, otherwise it is too difficult
             * to handle the errors correctly.
             *
             * To handle the gamma correction work out the upper and lower
             * bounds of the digitized value.  Assume rounding here - normally
             * the values will be identical after this operation if there is
             * only one transform, feel free to delete the png_error checks on
             * this below in the future (this is just me trying to ensure it
             * works!)
             *
             * Interval arithmetic is exact, but to implement it it must be
             * possible to control the floating point implementation rounding
             * mode. This cannot be done in ANSI-C, so instead I reduce the 'lo'
             * values by DBL_EPSILON and increase the 'hi' values by the same.
             */
#                    define DD(v, d, r)                                        \
                        (digitize(v * (1 - DBL_EPSILON), d, r)                 \
                         * (1 - DBL_EPSILON))
#                    define DU(v, d, r)                                        \
                        (digitize(v * (1 + DBL_EPSILON), d, r)                 \
                         * (1 + DBL_EPSILON))
            ...
            if (data.gamma != 1)
            {
                ...
                /* If a gamma calculation is done it is done using lookup tables
                 * of precision gamma_depth, so the already digitized value
                 * above may need to be further digitized here.
                 */
             ...
             * coefficients depends on whether they were specified on the
             * command line (in which case truncation to 15 bits happened) or
             * not (rounding was used) the maximum error in an individual
             * coefficient is always 2/32768, because even in the rounding case
             * the requirement that coefficients add up to 32768 can cause a
             * larger rounding error.
             *
             * The only time when rounding doesn't occur in 1.5.5 and later is
             * when the non-gamma code path is used for less than 16 bit data.
             */
...
#                    if !RELEASE_BUILD
                ...
                if (err > pm->limit)
                {
                    ...
                    pos =
                        safecat(buffer, sizeof buffer, pos, " exceeds limit ");
...
#                    endif /* before 1.7 */
                if (data.gamma == 1)
                {
                    /* There is no need to do the conversions to and from linear
                     * space, so the calculation should be a lot more accurate.
                     * There is a built in error in the coefficients because
                     * they only have 15 bits and are adjusted to make sure they
                     * add up to 32768.  This involves a integer calculation
                     * with truncation of the form:
                     *
                     *     ((int)(coefficient * 100000) * 32768)/100000
                     *
                     * This is done to the red and green coefficients (the ones
                     * provided to the API) then blue is calculated from them so
                     * the result adds up to 32768.  In the worst case this can
                     * result in a -1 error in red and green and a +2 error in
                     * blue.  Consequently the worst case in the calculation
                     * below is 2/32768 error.
                     *
                     * TODO: consider fixing this in libpng by rounding the
                     * calculation limiting the error to 1/32768.
                     *
                     * Handling this by adding 2/32768 here avoids needing to
                     * increase the global error limits to take this into
                     * account.)
                     */
                    gray = r * data.red_coefficient + g * data.green_coefficient
                        + b * data.blue_coefficient;
                    err = re * data.red_coefficient
                        + ge * data.green_coefficient
                ...
                {
                    /* The calculation happens in linear space, and this
                     * produces much wider errors in the encoded space.  These
                     * are handled here by factoring the errors in to the
                     * calculation.  There are two table lookups in the
                     * calculation and each introduces a quantization error
                     * defined by the table size.
                     */
                    png_modifier *pm = display->pm;
                    double in_qe =
                        (that->sample_depth > 8 ? .5 / 65535 : .5 / 255);
                    double out_qe =
                        (that->sample_depth > 8
                             ? .5 / 65535
                    ...
                    /* Work out the lower and upper bounds for the gray value in
                     * the encoded space, then work out an average and error.
                     * Remove the previously added input quantization error at
                     * this point.
                     */
                    gray = r * data.red_coefficient + g * data.green_coefficient
                    ...
                    grayhi = rhi * data.red_coefficient
                        + ghi * data.green_coefficient
...
#                    if !RELEASE_BUILD
                    ...
                    if (err > pm->limit)
                    {
                        ...
                        pos = safecat(buffer, sizeof buffer, pos,
                                      " exceeds limit ");
                        pos =
                            safecatd(buffer, sizeof buffer, pos, pm->limit, 6);
                        png_warning(pp, buffer);
                        pm->limit = err;
                    }
#                    endif /* !RELEASE_BUILD */
                                                   ...
                                                   png_structp pp, png_infop pi)
{
    ...
    /* We need a background colour, because we don't know exactly what
     * transforms have been set we have to supply the colour in the original
     * file format and so we need to know what that is!  The background colour
     * is stored in the transform_display.
     */
    R8(random_bytes);

    /* Read the random value, for colour type 3 the background colour is
     * actually expressed as a 24bit rgb, not an index.
     */
                                       ...
                                       const transform_display *display)
{
    ...
    if (that->alphaf < 1)
    {
        ...
        else /* 0 < alpha < 1 */
        {
            ...
            that->rede =
                that->rede * that->alphaf + data.rede * alf + DBL_EPSILON;
            ...
            that->bluee =
                that->bluee * that->alphaf + data.bluee * alf + DBL_EPSILON;
        }
...
#                define image_transform_png_set_background_add                 \
...
static void image_transform_png_set_filler_mod(const image_transform *this,
                                               image_pixel *that,
                                               png_const_structp pp,
                                               const transform_display *display)
{
...
static void image_transform_png_set_shift_mod(const image_transform *this,
                                              image_pixel *that,
                                              png_const_structp pp,
                                              const transform_display *display)
{
                                       ...
                                       png_byte bit_depth)
{
    for (;;) /* until we manage to add something */
    {
         ...
         * of the list.  This routine always returns the current counter (not
         * the next) so it returns 0 at the end and expects 0 at the beginning.
         */
                ...
                && (max == 0 || list->local_use < max))
            {
                ...
                else
                {
                    ...
                    *this = list->next;
                    ...
                    /* And, since we know it isn't useful, stop it being added
                     * again in this run:
                     */
                    list->local_use = max;
                }
...
static void perform_transform_test(png_modifier *pm)
{
                       ...
                       pm->test_tRNS))
    {
        ...
        for (;;)
        {
            ...
            /* 'max' is currently hardwired to '1'; this should be settable on
             * the command line.
             */
            counter =
                image_transform_add(&list, 1 /*max*/, counter, name,
...
#            ifdef PNG_READ_ALPHA_MODE_SUPPORTED
        {
             ...
             * non-inverted, representation.  It provides a default for the PNG
             * file gamma, but since the file has a gAMA chunk this does not
             * matter.
             */
...
#                endif
            ...
            /* However, for the standard Porter-Duff algorithm the output
             * defaults to be linear, so if the test requires non-linear output
             * it must be corrected here.
             */
...
#            endif
        ...
        {
#            ifdef PNG_READ_BACKGROUND_SUPPORTED
            /* NOTE: this assumes the caller provided the correct background
             * gamma!
             */
...
#            endif
     ...
     * the actual sample values - unencoded light intensity values.  Note that
     * in practice these are not completely unencoded because they include a
     ...
     */
    {
        ...
        /* Convert to linear light (with the above caveat.)  The alpha channel
         * is already linear.
         */
        if (alpha >= 0)
        {
            ...
            tcompose = 0;
            input_sample = gamma_component_compose(
                do_background, input_sample, alpha, background, &tcompose);

            if (tcompose)
                compose = 1;
        }

        /* And similarly for the output value, but we need to check the
         * background handling to linearize it correctly.
         */
        output = od;
        output /= outmax;
...
#            endif
            {
                /* In all other cases the output alpha channel is linear
                 * already, don't log errors here, they are much larger in
                 * linear data.
                 */
                output_is_encoded = 0;
                log_max_error = 0;
...
#            endif
        ...
        if (encoded_error < vi->maxout_total)
        {
            ...
            /* Test passed but error is bigger than the log limit, record why
             * the test passed:
             */
         ...
         *  encoded_sample: encoded result; input_sample scaled to output bit
         * depth
         *
         * libpng calculations:
         ...
         */
        {
            ...
            if (tmp < vi->maxabs)
                tmp = vi->maxabs;
            /* If 'compose' is true the composition was done in linear space
             * using integer arithmetic.  This introduces an extra error of +/-
             * 0.5 (at least) in the integer space used.  'maxcalc' records
             * this, taking into account the possibility that even for 16 bit
             * output 8 bit space may have been used.
             */
            if (compose && tmp < vi->maxcalc)
                tmp = vi->maxcalc;

            /* The 'maxout' value refers to the encoded result, to compare with
             * this encode input_sample adjusted by the maximum error (tmp)
             * above.
             */
...

        {
            ...
            if (pass == 0 && vi->use_input_precision && vi->dp->sbit)
            {
                 ...
                 * libpng 16-to-8 processing.  Assume that the input value (id,
                 * adjusted for sbit) can be anywhere between value-.5 and
                 * value+.5 - quite a large range if sbit is low.
                 *
                 * NOTE: at present because the libpng gamma table stuff has
                 * been changed to use a rounding algorithm to correct errors in
                 * 8-bit calculations the precise sbit calculation (a shift) has
                 * been lost.  This can result in up to a +/-1 error in the
                 * presence of an sbit less than the bit depth.
                 */
#            if PNG_LIBPNG_VER < 10700
#                define SBIT_ERROR .5
#            else
...
#            endif
                ...
                tmp = gamma_component_compose(do_background, tmp, alpha,
                                              background, NULL);
                 ...
                 * option has been used and 'inaccurate' scaling is used then
                 * the bit reduction is obtained by simply using the top 8 bits
                 * of the value.
                 *
                 * This is only done for older libpng versions when the
                 * 'inaccurate' (chop) method of scaling was used.
                 */
...
#            endif
             ...
             * output depends on the background/composite operation being
             * performed because this changes what parameters were actually used
             * above.
             */
            {
                size_t pos = 0;
...
#            endif /* ALPHA_MODE_SUPPORTED */
                ...
                if (use_input || pass != 0 /*logging*/)
                {
                    ...
                    pos = safecat(msg, sizeof msg, pos, "/");
                    /* The output is either "id/max" or "id sbit(sbit):
                     * isbit/max" */
                ...
                if (compose || pass != 0)
                {
                     ...
                     * calculated linear value here (the code above doesn't
                     * record the input value before composition is performed,
                     * so what gets reported is the value after composition.)
                     */
                    if (use_input || pass != 0)
                    {
                        if (vi->file_inverse > 0)
                        {
                            pos = safecat(msg, sizeof msg, pos, "^");
                            pos = safecatd(msg, sizeof msg, pos,
                                           vi->file_inverse, 2);
                        }
                    ...
                    if (use_background)
                    {
                        ...
                        pos = safecatd(msg, sizeof msg, pos, background,
                                       precision);
                        pos = safecat(msg, sizeof msg, pos, "*");
                        pos = safecatd(msg, sizeof msg, pos, 1 - alpha,
                                       precision);
                    }
                }
                ...
                if (compose || alpha < 0 || pass != 0)
                {
                    pos = safecat(msg, sizeof msg, pos,
                                  pass != 0 ? " =\n\t" : " = ");
                    pos =
                        safecatd(msg, sizeof msg, pos, input_sample, precision);
                    ...
                    {
                        pos = safecat(msg, sizeof msg, pos, " ^");
                        pos = safecatd(msg, sizeof msg, pos, vi->screen_inverse,
                                       2);
                ...
                if ((!compose && alpha >= 0) || pass != 0)
                {
                     ...
                     * them.)  Output a much shorter warning/error message and
                     * report the overall gamma correction.
                     */
                    if (vi->gamma_correction > 0)
                    {
                        pos = safecat(msg, sizeof msg, pos, " ^");
                        pos = safecatd(msg, sizeof msg, pos,
                                       vi->gamma_correction, 2);
                        pos = safecat(msg, sizeof msg, pos,
                                      "(gamma correction) =");
                    }
            ...
            for (x = 0; x < w; ++x)
            {
                    ...
                    || (in_ct == 3 && in_is_transparent))
                {
                    ...
                    if (output_alpha != 65536)
                        alpha = gamma_component_validate(
                            "alpha", &vi, input_alpha, output_alpha,
                            -1 /*alpha*/, 0 /*background*/);

                    else /* no alpha in output */
                    {
                        /* This is a copy of the calculation of 'i' above in
                         * order to have the alpha value to use in the
                ...
                else if (process_tRNS)
                {
                     ...
                     * currently 1 and needs to be 0 for an input pixel which
                     * matches the values in tRNS.
                     */
                    switch (in_ct)
                    {
                    case 0: /* gray */
...
static void perform_gamma_threshold_tests(png_modifier *pm)
{
    ...
    while (next_format(&colour_type, &bit_depth, &palette_number,
        ...
        if (palette_number < 2)
        {
            ...
            while (test_gamma >= .4)
            {
                /* There's little point testing the interlacing vs
                 * non-interlacing, but this can be set from the command line.
                 */
                gamma_threshold_test(pm, colour_type, bit_depth,
                                     pm->interlace_type, test_gamma,
                                     1 / test_gamma);
                test_gamma *= .95;
            }
                       ...
                       pm->test_lbg_gamma_transform, pm->test_tRNS))
    {
        ...
        for (i = 0; i < pm->ngamma_tests; ++i)
        {
            for (j = 0; j < pm->ngamma_tests; ++j)
            {
                if (i != j)
                {
                    gamma_transform_test(pm, colour_type, bit_depth,
                                         palette_number, pm->interlace_type,
                                         1 / pm->gammas[i], pm->gammas[j],
                                         0 /*sBIT*/, pm->use_input_precision,
                                         0 /*do not scale16*/);
...
static void perform_gamma_sbit_tests(png_modifier *pm)
{
    ...
    /* The only interesting cases are colour and grayscale, alpha is ignored
     * here for overall speed.  Only bit depths where sbit is less than the bit
     * depth are tested.
     */
                    ...
                    for (j = 0; j < pm->ngamma_tests; ++j)
                    {
                        if (i != j)
                        {
                            gamma_transform_test(
                                pm, colour_type, bit_depth, npalette,
...
#                endif
     ...
     * proceed *without* gamma correction, and the tests above will fail (but
     * not by much) - this could be fixed, it only appears with the -g option.
     */
    unsigned int i, j;
    for (i = 0; i < pm->ngamma_tests; ++i)
    {
        ...
        {
            if (i != j
                && fabs(pm->gammas[j] / pm->gammas[i] - 1)
                    >= PNG_GAMMA_THRESHOLD)
            {
                gamma_transform_test(
                    pm, 0, 16, 0, pm->interlace_type, 1 / pm->gammas[i],
                    pm->gammas[j], SBIT_16_TO_8, pm->use_input_precision_16to8,
                    1 /*scale16*/);
                ...
                gamma_transform_test(
                    pm, 2, 16, 0, pm->interlace_type, 1 / pm->gammas[i],
                    pm->gammas[j], SBIT_16_TO_8, pm->use_input_precision_16to8,
                    1 /*scale16*/);
                ...
                gamma_transform_test(
                    pm, 4, 16, 0, pm->interlace_type, 1 / pm->gammas[i],
                    pm->gammas[j], SBIT_16_TO_8, pm->use_input_precision_16to8,
                    1 /*scale16*/);
                ...
                gamma_transform_test(
                    pm, 6, 16, 0, pm->interlace_type, 1 / pm->gammas[i],
                    pm->gammas[j], SBIT_16_TO_8, pm->use_input_precision_16to8,
                    1 /*scale16*/);
...
#                endif
    ...
    if (expand_16 || bit_depth == 16)
    {
        ...
        /* In earlier libpng versions, those where DIGITIZE is set, any
         * background gamma correction in the expand16 case was done using 8-bit
         * gamma correction tables, resulting in larger errors.  To cope with
         * those cases use a 16-bit background value which will handle this
         * gamma correction.
         */
#                if DIGITIZE
        if (expand_16
...
#                endif
    ...
    if (do_background < ALPHA_MODE_OFFSET)
    {
        ...
        pos = safecat(name, sizeof name, pos, "(");
        /* This assumes no expand gray->rgb - the current code won't handle
         * that!
         */
        if (colour_type & PNG_COLOR_MASK_COLOR)
        {
    ...
    gamma_test(pm, colour_type, bit_depth, palette_number, interlace_type,
               file_gamma, screen_gamma, 0 /*sBIT*/, 0, name,
               use_input_precision, 0 /*strip 16*/, expand_16, do_background,
               &background, bg);
}
                                            ...
                                            int expand_16)
{
    ...
    /* Skip the non-alpha cases - there is no setting of a transparency colour
     * at present.
     *
            ...
            for (i = 0; i < pm->ngamma_tests; ++i)
            {
                for (j = 0; j < pm->ngamma_tests; ++j)
                {
                    gamma_composition_test(
                        pm, colour_type, bit_depth, palette_number,
...
static void perform_interlace_macro_validation(void)
{
    ...
    for (pass = 0; pass < 7; ++pass)
    {
        ...
        for (v = 0;;)
        {
             ...
             * the possible range for a 32-bit result.  In fact the values
             * should never be outside the range for a 31-bit result, but
             * checking for 32 bits here ensures that if an app uses a bogus
             * pass row or column (just so long as it fits in a 32 bit integer)
             * it won't get a possibly dangerous overflow.
             */
...
#    ifdef HAVE_FEENABLEEXCEPT
     ...
     * disabled; this is not necessary but if it is not done pngvalid will
     * likely end up ignoring FP conditions that other platforms fault.
     */
...
#    endif
     ...
     * overwrite checking.
     */
    store_ensure_image(&pm.this, NULL, 2, TRANSFORM_ROWMAX,
                       TRANSFORM_HEIGHTMAX);
     ...
     * code that 16-bit arithmetic is used for 8-bit samples when it would make
     * a difference.
     */
...
#    endif
    ...
    /* The following allows results to pass if they correspond to anything in
     * the transformed range [input-.5,input+.5]; this is is required because of
     * the way libpng treats the 16_TO_8 flag when building the gamma tables in
...
#    endif
     ...
     * maximum 16 bit error is about +/-1.9 in the fixed point implementation
     * but this is only allowed for values >38149 by the following:
     */
    ...
    while (--argc >= 1)
    {
        ...
        if (strcmp(*argv, "-v") == 0)
            ...
            pm.this.speed = 1, pm.ngamma_tests = pm.ngammas,
...
#    ifdef PNG_READ_TRANSFORMS_SUPPORTED
                         ...
                         "--transform-disable=", sizeof "--transform-disable")
                 == 0)
        {
            pm.test_transform = 1;
            transform_disable(*argv + sizeof "--transform-disable");
                         ...
                         "--transform-enable=", sizeof "--transform-enable")
                 == 0)
        {
            pm.test_transform = 1;
...
#    endif /* PNG_SET_OPTION_SUPPORTED */
    ...
    if (pm.test_standard == 0 && pm.test_size == 0 && pm.test_transform == 0
        && pm.ngamma_tests == 0)
    {
        /* Make this do all the tests done in the test shell scripts with the
         * same parameters, where possible.  The limitation is that all the
         * progressive read and interlace stuff has to be done in separate runs,
         * so only the basic 'standard' and 'size' tests are done.
         */
        pm.test_standard = 1;
        pm.test_size = 1;
        pm.test_transform = 1;
