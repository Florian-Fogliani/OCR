...
typedef struct
{
    /* The components, for grayscale images the gray value is in 'g' and if
     * alpha is not present 'a' is set to 255 or 65535 according to format.
     */
    int r, g, b, a;
} Pixel;
...
#    if defined(PNG_FORMAT_AFIRST_SUPPORTED)                                   \
        && defined(PNG_FORMAT_BGR_SUPPORTED)
...
static void gpc_bckg(Pixel *out, const Pixel *in, const Background *back)
{
    ...
    else
    {
        ...
        out->r = out->g = out->b =
            sRGB(sRGB_to_d[in->g] * a + back->dg * (1 - a));
    }
...
static void gpc_sGp(Pixel *out, const Pixel *in, const Background *back)
{
    ...
    {
        out->r = out->g = out->b =
            sRGB(YfromRGBint(in->r, in->g, in->b) / in->a);
        out->a = u8d(in->a / 257.);
    }
}
...
static void (*const gpc_fn[8 /*in*/][8 /*out*/])(Pixel *out, const Pixel *in,
    ...
    { gpc_bckg, gpc_noop, gpc_bckc, gpc_noop, gpc_preq, gpc_pre, gpc_preq,
      gpc_pre },
    { gpc_g8, gpc_g8, gpc_noop, gpc_noop, gpc_glin, gpc_glin, gpc_lin,
      gpc_lin },
    ...
    { gpc_sRGB, gpc_sRGB, gpc_sRGB, gpc_sRGB, gpc_noop, gpc_noop, gpc_noop,
      gpc_noop },
    { gpc_b16g, gpc_unpg, gpc_b16c, gpc_unpc, gpc_A, gpc_noop, gpc_A,
      gpc_noop },
    { gpc_sG, gpc_sG, gpc_sRGB, gpc_sRGB, gpc_g16, gpc_g16, gpc_noop,
      gpc_noop },
                                   ...
                                   png_const_colorp background, int via_linear)
{
    ...
    if (via_linear)
    {
        ...
        result->from_linear =
            gpc_fn[out_base | PNG_FORMAT_FLAG_LINEAR][out_base];
    ...
    else if (~in_format & out_format & PNG_FORMAT_FLAG_COLORMAP)
    {
         ...
         * typically be large (only the grayscale-no-alpha case permits
         * preserving even 8-bit values.)
         */
    ...
    else
    {
         ...
         * currently contains a different implementation for mapping a colormap
         * if both input and output are colormapped we need different conversion
    ...
    /* Follow the libpng simplified API rules to work out what to pass to the
     * gpc routines as a background value, if one is not required pass NULL so
     * that this program crashes in the even of a programming error.
     */
    ...
    {
        /* The input value is 'NULL' to use the background and (otherwise) an
         * sRGB background color (to use a solid color).  The code above uses a
         * fixed byte value, BUFFER_INIT8, for buffer even for 16-bit output.
         * For linear (16-bit) output the sRGB background color is ignored; the
         * composition is always on the background (so BUFFER_INIT8 * 257),
         * except that for the colormap (i.e. linear colormapped output) black
         * is used.
         */
        ...
        else /* sRGB output */
        {
            if (background != NULL)
            {
                if (out_format & PNG_FORMAT_FLAG_COLOR)
                {
                    ...
                    /* TODO: sometimes libpng uses the power law conversion
                ...
                else /* grayscale: libpng only looks at 'g' */
                {
                     ...
                     */
                    result->background_color.dr = result->background_color.dg =
                        result->background_color.db =
                            sRGB_to_d[background->green];
                }
            }
            ...
            else if ((out_format & PNG_FORMAT_FLAG_COLORMAP) == 0)
            {
                ...
                /* TODO: sometimes libpng uses the power law conversion here,
                 * how to handle this?
                 */
                result->background_color.dr = result->background_color.dg =
...
static void print_pixel(char string[64], const Pixel *pixel, png_uint_32 format)
{
    ...
    case PNG_FORMAT_FLAG_COLOR:
        sprintf(string, "%s(%d,%d,%d)", format_names[format], pixel->r,
                pixel->g, pixel->b);
        break;

    case PNG_FORMAT_FLAG_COLOR | PNG_FORMAT_FLAG_ALPHA:
        sprintf(string, "%s(%d,%d,%d,%d)", format_names[format], pixel->r,
                    ...
                    const char *reason)
{
    ...
    if (transform->in_image->file_name != transform->out_image->file_name)
    {
        ...
        sprintf(error_buffer,
                ...
                pixel_loc, reason, via_linear, pixel_in, background_info,
                pixel_out, pixel_calc);
    ...
    else
    {
        ...
        sprintf(error_buffer,
                "(%s) %s error%s:\n %s%s ->\n       %s\n  not: %s.\n"
                " The error happened when reading the original file with this "
                "format.",
                pixel_loc, reason, via_linear, pixel_in, background_info,
                pixel_out, pixel_calc);
        return logerror(transform->in_image, transform->in_image->file_name,
                    ...
                    png_uint_32 y /*or palette index*/)
{
    ...
    /* Check for alpha errors first; an alpha error can damage the components
     * too so avoid spurious checks on components if one is found.
     */
    errmsg = NULL;
    {
    ...
    return logpixel(transform, x, y, &pixel_in, &pixel_calc, &pixel_out,
                    errmsg);
}
                              ...
                              png_const_colorp background)
{
    ...
    if (formata & formatb & PNG_FORMAT_FLAG_COLORMAP)
    {
        ...
        /* Do this the slow way to accumulate the 'in_use' flags, don't break
         * out of the loop until the end; this validates the color-mapped data
         * to ensure all pixels are valid color-map indexes.
         */
        ...
        if (match)
        {
            ...
            for (y = 0; y < 256; ++y, a_cmap += a_sample, b_cmap += b_sample)
                if (in_use[y])
                {
                    /* The colormap entries should be valid, but because libpng
                     * doesn't do any checking at present the original image may
                     * contain invalid pixel values.  These cause an error here
                     * (at present) unless accumulating errors in which case the
                     * program just ignores them.
                     */
                    ...
                    else if (y >= b->image.colormap_entries)
                    {
                        if ((b->opts & ACCUMULATE) == 0)
                        {
                            ...
                            logerror(b, b->file_name,
                                     ": bad pixel index: ", pindex);
                        }
                        result = 0;
                    }

                    /* All the mismatches are logged here; there can only be
                     * 256! */
        ...
        /* else the image buffers don't match pixel-wise so compare sample
         * values instead, but first validate that the pixel indexes are in
         * range (but only if not accumulating, when the error is ignored.)
         */
...
#    endif
     ...
     *
     * If an alpha channel has been *added* then it must have the relevant
     * opaque value (255 or 65535).
     *
             ...
             & PNG_FORMAT_FLAG_COLOR)))
    {
         ...
         */
        {
             ...
             * 'bchannels' is a flag for matching formats.  btoa[x] says, for
             * each channel in b, where to find the corresponding value in a,
             * for the bchannels.  achannels may be different for a gray to rgb
             * transform (a will be 1 or 2, b will be 3 or 4 channels.)
             */
            ...
            if (alpha_added)
                alpha_added =
                    bloc[0]; /* location of alpha channel in image b */
            ...
            if (alpha_removed)
                alpha_removed =
                    aloc[0]; /* location of alpha channel in image a */
    ...
    {
        /* Direct compare is not possible, cancel out all the corresponding
         * local variables.
         */
        bchannels = 0;
    ...
    for (y = 0; y < height; ++y, rowa += stridea, rowb += strideb)
    {
        ...
        for (x = 0, ppa = rowa, ppb = rowb; x < width; ++x)
        {
            ...
            if (bchannels)
            {
                 ...
                 * alpha_removed value is set (not 4) then also do this only if
                 * the 'a' alpha channel (alpha_removed) is opaque; only
                 * relevant for the 8-bit case.
                 */
                if (formatb & PNG_FORMAT_FLAG_LINEAR) /* 16-bit checks */
                {
                    png_const_uint_16p pua =
                        aligncastconst(png_const_uint_16p, psa);
                    png_const_uint_16p pub =
                        aligncastconst(png_const_uint_16p, psb);
            ...
            /* If we get to here the fast match failed; do the slow match for
             * this pixel.
             */
                     ...
                     png_const_colorp background)
{
    ...
    if (image->input_memory != NULL)
    {
        if (!png_image_begin_read_from_memory(
                &image->image, image->input_memory, image->input_memory_size))
...
#    else
    ...
    {
        return logerror(image,
                        "unsupported file/stdio init: ", image->file_name, "");
    }
#    endif
     ...
     */
    {
        ...
        if ((format & FORMAT_NO_CHANGE) != 0)
        {
            if ((format & PNG_FORMAT_FLAG_COLORMAP) != 0
                ...
                format =
                    (image_format & ~BASE_FORMATS) | (format & BASE_FORMATS);
        ...
        image->stride =
            PNG_IMAGE_ROW_STRIDE(image->image) + image->stride_extra;
        allocbuffer(image);
...
#    endif
                        {
                            png_bytep b =
                                voidcast(png_bytep, malloc((size_t)cb));
...
#            endif
        ...
        if (f != NULL)
        {
                    ...
                    (png_int_32)image->stride, image->colormap))
            {
                ...
                {
                    rewind(f);
                    initimage(output, image->opts, "tmpfile",
                              image->stride_extra);
                    output->input_file = f;
...
#        endif /* SIMPLIFIED_WRITE_STDIO */
                ...
                (png_int_32)image->stride, image->colormap))
        {
            ...
            if (output->input_memory != NULL)
            {
                ...
                if (png_image_write_to_memory(
                        &image->image, output->input_memory,
                        &output->input_memory_size, convert_to_8bit,
                        image->buffer + 16, (png_int_32)image->stride,
                        image->colormap))
                {
                    /* This is also non-fatal but it safes safer to error out
                     * anyway:
                     */
    ...
    /* 'output' has an initialized temporary image, read this back in and
     * compare this against the original: there should be no change since the
     * original format was written unmodified unless 'convert_to_8bit' was
     * specified. However, if the original image was color-mapped, a simple read
     * will zap the linear, color and maybe alpha flags, this will cause
     * spurious failures under some circumstances.
     */
...

    {
        ...
        for (counter = 0; counter < 2 * FORMAT_COUNT; ++counter)
            if (format_isset(pf, counter >> 1))
            {
                ...
                /* If there is a format change that removes the alpha channel
                 * then the background is relevant.  If the output is 8-bit
                 * color-mapped then a background color *must* be provided,
                 * otherwise there are two tests to do - one with a color, the
                 * other with NULL.  The NULL test happens second.
                 */
                if ((counter & 1) == 0)
                {
                        ...
                        && (image->image.format & PNG_FORMAT_FLAG_ALPHA) != 0)
                    {
                        ...
                        /* BUT if the output is to a color-mapped 8-bit format
                         * then the background must always be a color, so
                         * increment 'counter' to skip the NULL test.
                         */
                        if ((format & PNG_FORMAT_FLAG_COLORMAP) != 0
                    ...
                    /* Otherwise an alpha channel is not being eliminated, just
                     * leave background NULL and skip the (counter & 1) NULL
                     * test.
                     */
                    else
                        ++counter;
                ...
                result = compare_two_images(image, &copy, 0 /*via linear*/,
                                            background);
                if (!result)
                    break;

#    ifdef PNG_SIMPLIFIED_WRITE_SUPPORTED
                /* Write the *copy* just made to a new file to make sure the
                 * write side works ok.  Check the conversion to sRGB if the
                 * copy is linear.
                 */
                output.opts = opts;
                ...
                /* Validate against the original too; the background is needed
                 * here as well so that compare_two_images knows what color was
                 * used.
                 */
                ...
                {
                    /* 'output' is linear, convert to the corresponding sRGB
                     * format.
                     */
                    output.opts = opts;
                    result =
                        write_one_file(&output, &copy, 1 /*convert to 8bit*/);
                    if (!result)
                        break;

                    /* This may involve a conversion via linear; in the ideal
                     * world this would round-trip correctly, but libpng 1.5.7
                     * is not the ideal world so allow a drift
                     * (error_via_linear).
                     *
                     * 'image' has an alpha channel but 'output' does not then
                     * there will a strip-alpha-channel operation (because
                     * 'output' is linear), handle this by composing on black
                     * when doing the comparison.
                     */
                    result = compare_two_images(image, &output,
                                                1 /*via_linear*/, background);
                    if (!result)
                        break;
                }
...
#    endif
        ...
        else
        {
            ...
            if (arg[0] == '-')
            {
                ...
                /* Loop reading files, use a static buffer to simplify this and
                 * just stop if the name gets to long.
                 */
                ...
                do
                {
                     ...
                     * or, for -0, just '\0' (use -print0 to find to make this
                     * work!)
                     */
                    if (ch == EOF || ch == term || ch == 0)
                    {
                        ...
                        if (ich > 0
                            && !test_one_file(buffer, &formats, opts,
            ...
            else if (!test_one_file(arg, &formats, opts, stride_extra,
                                    log_pass))
                retval = 1;
        }
    }
