 ...
 */
#if defined(PNG_SET_UNKNOWN_CHUNKS_SUPPORTED) && defined(PNG_STDIO_SUPPORTED)  \
...
#            define png_eXIf                                                   \
                PNG_U32(101, 88, 73, 102) /* registered July 2017 */
#            define png_fRAc                                                   \
        ...
        while (ntext > 0)
            ...
            default:
                fprintf(stderr, "%s(%s): unknown text compression %d\n",
                        d->file, d->test, text[ntext].compression);
                display_exit(d);
            }
    }
...
static int PNGCBAPI read_callback(png_structp pp, png_unknown_chunkp pc)
{
    ...
    /* However if there is no support to store unknown chunks don't ask libpng
     * to do it; there will be an png_error.
     */
#            ifdef PNG_SAVE_UNKNOWN_CHUNKS_SUPPORTED
    return discard;
#            else
...
static png_uint_32 get_unknown(display *d, png_infop info_ptr, int after_IDAT)
{
...

    {
        ...
        int num_unknown =
            png_get_unknown_chunks(d->png_ptr, info_ptr, &unknown);
        ...
        while (--num_unknown >= 0)
        {
             ...
             * must also be unknown to libpng the 'display->keep' behavior
            ...
            if (chunk < 0)
                ...
                case PNG_HANDLE_CHUNK_IF_SAFE:
                    ...
                    {
                        fprintf(stderr,
                                "%s(%s): if-safe: %s: unknown critical chunk "
                                "saved\n",
                 ...
                 png_uint_32p flags /*out*/, display *d, int set_callback)
{
     ...
     */
    d->png_ptr =
        png_create_read_struct(PNG_LIBPNG_VER_STRING, d, error, warning);
    if (d->png_ptr == NULL)
    {
        ...
        if (equals != NULL)
        {
            ...
            case 4: /* chunk name */
                ...
                if (chunk >= 0)
                {
                     ...
                     * if SAVE support is unavailable - we can't do a useful
                     * test in this case, so we just check the arguments!  This
                     * could be improved in the future by using the read
                     * callback.
                     */
#        if PNG_LIBPNG_VER >= 10700                                            \
            && !defined(PNG_SAVE_UNKNOWN_CHUNKS_SUPPORTED)
...
#        endif /* 1.7+ SAVE_UNKNOWN_CHUNKS */
                    {
                        ...
                        png_set_keep_unknown_chunks(d->png_ptr, option, name,
                                                    1);
...
#        if PNG_LIBPNG_VER >= 10700                                            \
            && !defined(PNG_SAVE_UNKNOWN_CHUNKS_SUPPORTED)
                    if (option < PNG_HANDLE_CHUNK_IF_SAFE)
#        endif /* 1.7+ SAVE_UNKNOWN_CHUNKS */
                        png_set_keep_unknown_chunks(d->png_ptr, option, NULL,
                                                    0);
                             ...
                             int set_callback)
{
    ...
    /* Chunks should either be known or unknown, never both and this should
     * apply whether the chunk is before or after the IDAT (actually, the app
     * can probably change this by swapping the handling after the image, but
...
#        endif
    ...
    else
    {
        ...
        /* Set the exit_test pointer here so we can continue after a libpng
         * error. NOTE: this leaks memory because the png_struct data from the
         * failing test is never freed.
         */
        while (*test)
        {
