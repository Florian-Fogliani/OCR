...
#    define COLOR_MASK_ALL                                                     \
        (PALETTE + COLOR + ALPHA) /* absent = gray, no alpha */
    png_byte bit_depths;
...
{
    return transforms
        & (transforms - 1); /* non-zero if more than one set bit */
}
 ...
 */
{
     ...
     */
    {
        ...
        /* Some transforms appear multiple times in the table; the 'active'
         * status is the logical OR of these and the inactive status must be
...
#    define C(item)                                                            \
    ...
    /* All the chunks from the original PNG should be preserved in the output
     * PNG because the PNG format has not been changed.
     */
    {
    ...
    if (rowbytes != dp->original_rowbytes)
        display_log(dp, APP_ERROR, "PNG rowbytes changed from %lu to %lu",
                    (unsigned long)dp->original_rowbytes,
            ...
            || color_type == PNG_COLOR_TYPE_PALETTE)
        {
            ...
            for (y = 0; y < height; ++y)
            {
                            ...
                            != (orig[rowbytes - 1] & mask))))
                {
                    ...
                    display_log(
                        dp, APP_FAIL, "byte(%lu,%lu) changed 0x%.2x -> 0x%.2x",
...
#    ifdef PNG_sBIT_SUPPORTED
        {
...

            {
                ...
                for (b = 0; 8 * b < bpp; ++b)
                {
                    /* libpng should catch this; if not there is a security
                     * issue because an app (like this one) may overflow an
                     * array. In fact libpng doesn't catch this at present.
                     */
                    if (sig_bits[b] == 0
                        || sig_bits[b] > bit_depth /*!palette*/)
                        display_log(dp, LIBPNG_BUG,
                                    "invalid sBIT[%u]  value %d returned for "
                                    "PNG bit depth %d",
            ...
            {
                /* sanity check; this is a grayscale PNG; something is wrong in
                 * the code above.
                 */
                display_log(dp, INTERNAL_ERROR,
                            "invalid bpp %u for bit_depth %u", bpp, bit_depth);
            }
            ...
            if (mask != 0)
            {
                ...
                if (bpp != 1 || mask == 0)
                    display_log(dp, INTERNAL_ERROR,
                                "mask calculation error %u, %u", bpp, mask);
            }
            ...
            for (y = 0; y < height; ++y)
            {
                ...
                {
                    display_log(dp, APP_FAIL,
                                "significant bits at (%lu[end],%lu) changed", x,
                                y);
                    return 0;
                }
            } /* for y */
...
#        endif
     ...
     */
    if (transforms
        & (PNG_TRANSFORM_PACKING | PNG_TRANSFORM_STRIP_FILLER
...
#    endif
     ...
     */
    {
        ...
        for (;;)
        {
            ...
            /* If this involved any irreversible transformations then if we
             * write it out with just the reversible transformations and read it
             * in again with the same transforms we should get the same thing.
             * At present this isn't done - it just seems like a waste of time
...
#    ifdef PNG_WRITE_PNG_SUPPORTED
            ...
            {
                /* All transforms reversible: write the PNG with the
                 * transformations reversed, then read it back in with no
                 * transformations.  The result should be the same as the
                 * original apart from the loss of low order bits because of the
                 * SHIFT/sBIT transform.
                 */
                dp->operation = "reversible transforms";
                write_png(dp, dp->read_ip, current);

                /* And if this is read back in, because all the transformations
                 * were reversible, the result should be the same.
                 */
                read_png(dp, &dp->written_file, NULL, 0);
                if (!compare_read(dp, current /*for the SHIFT/sBIT transform*/))
                {
                    /* This set of transforms failed.  If a single bit is set -
                     * if there is just one transform - don't include this in
                     * further 'exhaustive' tests.  Notice that each transform
                     * is tested on its own before testing combos in the
                     * exhaustive case.
                     */
                    if (is_combo(current))
                    {
