...
int validation_ascii_to_fp(int count, int argc, char **argv)
{
    ...
    do
    {
        ...
        else
        {
            ...
            double allow = .5
                / pow(10, (precision >= DBL_DIG) ? DBL_DIG - 1 : precision - 1);
            ...
            {
                double percent =
                    (precision >= DBL_DIG) ? max_error_abs : max_error;
                ...
                {
                    fprintf(stderr,
                            "%.*g[%d] -> '%s' -> %.*g number changed (%g > %g "
                            "(%d%%))\n",
                            DBL_DIG, test, precision, buffer, DBL_DIG, out,
                            change, allow, (int)round(allowp));
                    failed = 1;
                }
                else
                    ++minorarith;
    ...
    skip:
        ...
        else
        {
            ...
            int exponent =
                precision % (DBL_MAX_EXP - DBL_MIN_EXP) + DBL_MIN_EXP;
            int tmp;
...
int validation_checkfp(int count, int argc, char **argv)
{
    ...
    printf("checkfp: %s: checked %d,%.3d,%.3d,%.3d strings (%d invalid)\n",
           ...
           command.cmillions % 1000, command.ctimes / 1000,
           command.ctimes % 1000, command.cinvalid);
