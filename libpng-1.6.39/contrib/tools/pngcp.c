...
#        define IGNORE_INDEX                                                   \
            0x400 /* Ignore out of range palette indices (BAD!) */
#        ifdef PNG_GET_PALETTE_MAX_SUPPORTED
#            define FIX_INDEX                                                  \
                0x800 /* 'Fix' out of range palette indices (OK) */
...
#    define MAX_SIZE ((png_alloc_size_t)(-1))
    ...
    /* Values used while iterating through all the combinations of settings for
     * a single file:
     */
 ...
 */
{
    ...
    if (level > (error_level)(dp->options & LEVEL_MASK))
    {
        ...
        fprintf(stderr, "%s: %s: %s",
                dp->filename != NULL ? dp->filename : "<stdin>", lp,
                dp->operation);
...
static void text_stash(struct display *dp)
{
     ...
     * make a copy of the text chunks and png_set_text it each time.  This adds
     * a text chunks, so they get replicated, but only the new set gets written
    ...
    {
        dp->text_ptr =
            voidcast(png_textp, malloc(dp->num_text * sizeof *chunks));
 ...
 */
{
    ...
    for (j = 0; j < option_count; ++j)
        if (strncmp(options[j].name, opt, len) == 0
            && options[j].name[len] == 0)
            return j;
...
/* Add the appropriate option string to dp->curr. */
{
    ...
    if (entry_name == range_lo)
        ...
        add = sprintf(dp->curr + offset, " --%s=%s", options[opt].name,
                      entry_name);
 ...
 */
{
    ...
    if (sp >= dp->tsp)
    {
         ...
         * searched entry or the start of the dp->curr buffer if there is
         * nothing on the stack yet (sp == 0).
         */
 ...
 */
{
    ...
    {
        /* A range can be iterated over or searched.  The default iteration
         * option is indicated by hi < lo on the stack, otherwise the range
         * being search is [lo..hi] (inclusive).
         */
        ...
        else
        {
            ...
            else
            {
                ...
                /* These cases arise with the 'probe' handling below when there
                 * is a dip or peak in the size curve.
                 */
                ...
                /* The following should be true or something got messed up
                 * above. */
                assert(lo < val && val < hi);

                /* If there are only four entries (lo, val, hi plus one more)
                 * just test the remaining entry.
                 */
                if (hi == lo + 3)
                {
                    /* Because of the 'probe' code val can either be lo+1 or
                     * hi-1; we need to test the other.
                     */
                    val = lo + ((val == lo + 1) ? 2 : 1);
                    assert(lo < val && val < hi);
                    dp->stack[sp].end = 1;
                ...
                {
                    /* There are at least 2 entries still untested between lo
                     * and hi, i.e. hi >= lo+4.  'val' is the midpoint +/- 0.5
                     *
                    ...
                    {
                        /* Select the low range; testing this first favours the
                         * low range over the high range when everything comes
                         * out equal. Because of the probing 'val' may be lo+1.
                         * In that case end the search and set 'val' to lo+2.
                         */
                        if (val == lo + 1)
                        {
                            ++val;
                    ...
                    {
                        /* Monotonically decreasing size; this is the expected
                         * case. Select the high end of the range.  As above,
                         * val may be hi-1.
                         */
                        if (val == hi - 1)
                        {
                    ...
                    /* If both those tests failed 'best_size' is either greater
                     * than or less than both lo_size and hi_size.  There is a
                     * peak or dip in the curve of sizes from lo to hi and val
                     * is on the peak or dip.
                     *
                     * Because the ranges being searched as so small (level
                     * is 1..9, windowBits 8..15, memLevel 1..9) there will only
                     * be at most three untested values between lo..val and
                     * val..hi, so solve the problem by probing down from hi or
                     * up from lo, whichever is the higher.
                     *
                     * This is the place where 'val' is set to outside the range
                     * lo..hi, described as 'probing', though maybe 'narrowing'
                    ...
                    /* lo and hi are still the true range limits, check for the
                     * end condition.
                     */
                    assert(hi > lo + 1);
                    if (hi <= lo + 2)
 ...
 */
{
    ...
    {
        /* Simple, single value, entries don't have a stack frame and have a
         * fixed value (it doesn't change once set on the command line).
         * Otherwise the value (entry) selected from the command line is 'all':
         */
...
{
    return getallopts_(dp, option_index(dp, opt_str, strlen(opt_str)), value,
                       0);
}
...
/* As above except that if the option was not set try a search */
{
    ...
    else if (opt == OPTIND(dp, windowBits))
    {
         ...
         * one byte, so given that the maximum string length is 258, a
         * windowBits of 9 is always sufficient.
         */
...
static int opt_check(struct display *dp, const char *arg)
{
    ...
    if (arg != NULL && arg[0] == '-' && arg[1] == '-')
    {
        ...
        /* So arg[0..i-1] is the argument name, this does not return if this
         * isn't a valid option name.
         */
        ...
        else
        {
                ...
                if (negate)
                {
                    ...
                    else
                    {
                        display_log(dp, USER_ERROR,
                                    "%.*s: option=arg cannot be negated", i,
                                    arg);
                        abort(); /* NOT REACHED */
                    }
                }
            ...
            {
                /* --no-option negates list values from the default, which
                 * should therefore be 'all'.  Notice that if the option list is
                 * empty in this case nothing will be removed and therefore
                 * --no-option= is the same as --option.
                 */
                if (negate)
        ...
        if (options[j].opt < LEVEL_MASK)
        {
            ...
            if (val) /* Set this level */
                ...
                display_log(dp, USER_ERROR,
                            "%.*s: messages cannot be turned off individually; "
                            "set a message level",
                            i, arg);
        }
...
static void start_timer(struct display *dp, int what)
{
    if ((dp->value[OPTIND(dp, time)] & what) != 0)
        set_timer(dp,
                  what == PNGCP_TIME_READ ? &dp->read_time : &dp->write_time);
}
    ...
    else
    {
        ...
        if (ret == 0) /* the entry exists */
        {
            ...
            if (access(pathname, W_OK) != 0)
                display_log(dp, USER_ERROR, "%s: cannot be written (%s)",
...
static void makename(struct display *dp, const char *dir, const char *infile)
{
    ...
    else
    {
        ...
        if (dsize <= (sizeof dp->namebuf) - 2) /* Allow for name + '/' + '\0' */
        {
            ...
            if (dsize + isize
                < (sizeof dp->namebuf)) /* dsize + infile + '\0' */
            {
                memcpy(dp->namebuf + dsize, infile, isize + 1);

                if (isdir(dp, dp->namebuf))
                    display_log(dp, USER_ERROR,
                                "%s: output file is a directory", dp->namebuf);
            }
            ...
            display_log(dp, USER_ERROR, "%s: output directory name too long",
                        dir);
    }
}
...
static void PNGCBAPI read_function(png_structp pp, png_bytep data, size_t size)
{
    struct display *dp = get_dp(pp);
...
{
    /* This is an assumption of the code; it may happen if a previous write
     * fails and there is a bug in the cleanup handling below (look for setjmp).
...
#    endif /* SET_USER_LIMITS */
    ...
    {
        /* png_get_rowbytes should never return 0 because the value is set by
         * the first call to png_set_IHDR, which should have happened by now,
         * but just in case:
         */
...
#    endif /* FIX_INDEX */
    ...
    /* NOTE: dp->ip is where all the information about the PNG that was just
     * read is stored.  It can be used to write and write again a single PNG
     * file, however be aware that prior to libpng 1.7 text chunks could only be
     * written once; this is a bug which would require a significant code
     * rewrite to fix, it has been there in several versions of libpng (it was
     * introduced to fix another bug involving duplicate writes of the text
     * chunks.)
     */
                        ...
                        const char *destname)
{
    ...
    if (dp->nsp > 0) /* iterating over lists */
    {
        ...
        do
        {
            /* Clean before each write_png; this just removes *dp->write_pp
                ...
                || (dp->write_size == dp->best_size && better_options(dp)))
            {
                if (destname != NULL && rename(tmpname, destname) != 0)
                    display_log(dp, APP_ERROR, "rename %s %s failed (%s)",
                                tmpname, destname, strerror(errno));
...
static int cppng(struct display *dp, const char *file, const char *gv dest)
...
int main(int argc, char **argv)
{
...

    {
        ...
        /* Do this at least once; if there are no arguments stdin/stdout are
         * used.
         */
        d.operation = "files";
        do
        {
            ...
            {
                /* This result may already have been output, in which case
                 * best_size has been reset.
                 */
            ...
            else if (d.options & SIZES)
            {
                printf(
                    ...
                    cts(d.ct), (unsigned long)d.size,
                    (unsigned long)d.read_size, (unsigned long)d.write_size,
                    (unsigned long)d.results);
                fflush(stdout);
            }

            /* Here on any return, including failures, except user/internal
             * issues
             */
            {
