...
double finvsRGB(unsigned int i)
{
...
int main(int argc, char **argv)
{
    ...
    /* Now work out the decoding tables (this is where the error comes in
     * because there are 512 set points and 512 straight lines between them.)
     */
    for (;;)
    {
        if (ec_lo == 0)
    ...
    for (ibase = 0; ibase < 65536; ibase += 128)
    {
        ...
        png_byte delta = png_sRGB_delta[ibase >> 7], trydelta = delta,
                 od = delta;
    ...
    for (i = 0; i <= max_input; ++i)
    {
        ...
        if (icalc != iexact)
        {
            ...
            {
                printf("/* 0x%08x: exact: %3d, got: %3d [tables: %08x, %08x] "
                       "(%f) */\n",
    ...
    for (i16 = 0; i16 <= 65535; ++i16)
    {
        ...
        if (icalc != iexact)
        {
            ...
            if (abs(icalc - iexact) > 1)
                printf("/* 0x%04x: exact: %3d, got: %3d [tables: %08x, %08x] "
                       "(%f) */\n",
