...
#                define PNG_U32(b1, b2, b3, b4)                                \
                    (PNG_32b(b1, 24) | PNG_32b(b2, 16) | PNG_32b(b3, 8)        \
                     | PNG_32b(b4, 0))
...
#                define png_fRAc                                               \
                    PNG_U32(102, 82, 65, 99) /* registered, not defined */
...
static int uarb_mult_digit(uarb acc, int a_digits, uarb num,
                           FIX_GCC int n_digits, png_uint_16 val)
...
static png_uint_32 crc_init_4(png_uint_32 value)
{
     ...
     * separate tables to parallelize the four byte operations, it only works
     * for a CRC of the first four bytes of the stream, but this is what happens
     * in the parser below where length+chunk-name is read and chunk-name used
     * to initialize the CRC.  Notice that the calculation here avoids repeated
...
struct file
{
    ...
    /* The following variables are used during reading to record the length,
     * type and data position of the *next* chunk or, right at the start, the
...
#            define STATE_IDAT 2 /* An IDAT stream is being written */
     ...
     * is returned by setting one or other of the above variables.  Note that
     * the relevant initializer is called by the allocator function.  The
     * alloc_ptr is used only by the implementation of the allocate function.
     */
    void *alloc_ptr;
...
static char type_char(png_uint_32 v)
{
     ...
     * to help in finding internal (programming) errors.  Note that the code
     * only ever considers the low 7 bits of the value (so it is not necessary
     * for the type_name function to mask of the byte.)
     */
    if (v & 32)
 ...
 */
{
    ...
    /* The chunk being read is typically identified by file->chunk or, if this
     * is NULL, by file->type.  This may be wrong if libpng reads ahead, but
     * this only happens with IDAT where libpng reads the header then jumps
     * around finding errors in the previous chunks.  We know that is happening
     * because we are at the start of the IDAT (i.e. no IDAT data has yet been
     * written.)
     *
     * SUMMARY FORMAT (stop):
     *
    ...
    if (file->global->quiet < 2) /* need two quiets to stop this. */
    {
        ...
        if (file->chunk != NULL)
            type =
                current_type(file, code); /* Gropes in struct chunk and IDAT */
        ...
        /* This only works one strerror at a time, because of the way strerror
         * is implemented.
         */
 ...
 */
{
    ...
    if (file->width < 1 || file->width > 0x7fffffff)
             ...
             */
            {
                ...
                for (pass = 0; pass <= 6; ++pass)
                {
                    ...
                    if (pw > 0)
                    {
                        ...
                        digits = uarb_mult_digit(
                            row_bytes, uarb_set(row_bytes, 7), row_width,
                            uarb_set(row_width, pw), pd);
                        digits = uarb_shift(row_bytes, digits, 3);
                        digits = uarb_inc(row_bytes, digits, 1);

                        /* Add row_bytes * pass-height to the file image_bytes
                         * field
                         */
                        image_digits = uarb_mult32(
                            file->image_bytes, image_digits, row_bytes, digits,
        ...
        case PNG_INTERLACE_NONE: {
            ...
            digits =
                uarb_mult_digit(row_bytes, uarb_set(row_bytes, 7), row_width,
                                uarb_set(row_width, file->width), pd);
            digits = uarb_shift(row_bytes, digits, 3);
            digits = uarb_inc(row_bytes, digits, 1);

            /* Set row_bytes * image-height to the file image_bytes field */
            file->image_digits = uarb_mult32(file->image_bytes, 0, row_bytes,
                                             digits, file->height);
        }
        break;
...
struct IDAT
{
    ...
    struct IDAT_list
        *idat_list_head; /* START of the list of IDAT information */
    struct IDAT_list
        *idat_list_tail; /* *END* of the list of IDAT information */
...
static void IDAT_end(struct IDAT **idat_var)
{
    ...
    *idat_var = NULL;
    ...
    /* Regardless of why the IDAT was killed set the state back to CHUNKS (it
     * may already be CHUNKS because the state isn't changed until process_IDAT
 ...
 */
{
    ...
    if (len == 0) /* use original chunk lengths */
    {
        ...
        /* Otherwise rechunk_length is called at the end of a chunk for the
    ...
    else /* rechunking */
    {
        ...
        if (len > have)
        {
            ...
            unsigned int j =
                idat->idat_count + 1; /* the next IDAT in the list */

            do
            {
                /* Add up the remaining bytes.  This can't overflow because the
                 * individual lengths are always <= 0x7fffffff, so when we add
                 * two of them overflow is not possible.
                 */
                assert(cur != NULL);
                ...
                for (;;)
                {
                    ...
                    /* If this was the end return the count of the available
                     * bytes */
...
/* Output a message given a zlib rc */
{
    if (zlib->global->errors)
    {
        ...
        fprintf(stderr, ": %szlib error: %d (%s) (%s)\n",
                unexpected ? "unexpected " : "", zlib->rc, zlib_rc(zlib),
                reason);
    }
}
...
static void zlib_end(struct zlib *zlib)
{
    ...
    if (!zlib->global->quiet)
    {
        ...
        {
            /* This is a zlib read error; the chunk will be skipped.  For an
             * IDAT stream this will also cause a fatal read error (via stop()).
             *
             * SUMMARY FORMAT:
             *
             ...
             * spaces replaced by '-'.  The compressed byte count indicates
             * where in the zlib stream the error occurred.
             */
            type_name(zlib->chunk->chunk_type, stdout);
...
#            define ZLIB_FATAL 2
{
    ...
    while (in_bytes < nbytes && endrc == ZLIB_OK)
    {
         ...
         * compression' (which results in a lot of copying) Z_BUF_ERROR can
         * happen immediately after all output has been flushed on the next
         * input byte. This is handled below when Z_BUF_ERROR is detected by
         * adding an output byte.
         */
        zlib->z.next_in = &bIn;
        zlib->z.avail_in = 1;
        ...
        /* Initially use Z_NO_FLUSH in an attempt to persuade zlib to look at
         * this byte without confusing what is going on with output.
         */
        flush = Z_NO_FLUSH;
        out_bytes = 0;
                                 ...
                                 zlib->z.avail_out = 1, ++out_bytes)
        {
            ...
            case Z_DATA_ERROR:
                ...
                if (zlib->z.msg != NULL
                    && strcmp(zlib->z.msg, "invalid distance too far back")
                        == 0)
                {
...
/* Like zlib_advance but also handles a stream of IDAT chunks. */
{
     ...
     * all the input data has been used.)  This function uses the value to
     * update the correct chunk length, so the problem should only ever be
     * detected once for each chunk.  zlib_advance outputs the error message,
     * though see the IDAT specific check below.
     */
    zlib->extra_bytes = 0;
        ...
        for (;;)
        {
            ...
            for (i = 0; i < count; ++i)
            {
                ...
                case ZLIB_STREAM_END: /* stop */
                    /* There may be extra chunks; if there are and one of them
                     * is not zero length output the 'extra data' message.  Only
                     * do this check if errors are being output.
                     */
            ...
            if (zlib_reset(&zlib, test_bits))
            {
                ...
                case ZLIB_TOO_FAR_BACK:
                    ...
                    if (min_bits > max_bits)
                    {
                        ...
                        if (zlib.z.msg == NULL)
                            zlib.z.msg =
                                PNGZ_MSG_CAST("invalid distance too far back");
                        zlib_message(&zlib, 0 /*stream error*/);
                        zlib_end(&zlib);
                        return 0;
                    }
 ...
 */
{
     ...
     * ancillary chunks (and not tRNS, which should probably have been a
     * critical chunk.)
     */
     ...
     * function the chunk will be skipped.  The rewrite is cancelled here so
     * that it can be set in the per-chunk processing.
     */
    ...
    /* Control reaches this point if the chunk must be skipped.  For chunks
     * other than IDAT this means that the zlib compressed data is fatally
     * damaged and the chunk will not be passed to libpng.  For IDAT it means
     * that the end of the IDAT stream has not yet been reached and we must
     * handle the next (IDAT) chunk.  If the LZ data in an IDAT stream cannot be
     * read 'stop' must be used to halt parsing of the PNG.
     */
    read_chunk(file);
    return;
 ...
 */
{
    ...
    {
        /* Ignore the recorded chunk length, proceed through the data looking
         * for a leading sequence of bytes that match the CRC in the following
         * four bytes.  Each time a match is found check the next 8 bytes for a
         * valid length, chunk-type pair.
         */
        png_uint_32 length;
        png_uint_32 type = file->type;
        ...
        for (length = 0; length <= 0x7fffffff; ++length)
        {
            ...
            {
                /* A match on the CRC; for IEND this is sufficient, but for
                 * anything else expect a following chunk header.
                 */
                if (type == png_IEND)
                {
                    file->length = length;
                ...
                else
                {
                    ...
                    /* Examine the 8 bytes for a valid chunk header. */
                    {
                        ...
                        if (next_length < 0x7fffffff)
                        {
                            ...
                            {
                                file->read_count -= 8;
                                process_chunk(file, file_crc, next_length,
                                              next_type);
                                return;
                            }
                        }
             ...
             * the chunk_type is IEND the buffer will never be used, if it is
             * not the code will always read ahead exactly 8 bytes and pass this
             * on to process_chunk.  So the invariant that IEND leaves the file
             * position after the IEND CRC and other chunk leave it after the
             * *next* chunk header is not broken.
             */
            if (nread <= nused)
            {
                ch = read_byte(file);
        ...
        /* Control gets to here if when 0x7fffffff bytes (plus 8) have been
         * read, ok, treat this as a damaged stream too:
         */
    }
        ...
        if (nread == 4)
        {
            if (type != png_IEND) /* do not read beyond IEND */
            {
                ...
                if (nread == 8 && next_length <= 0x7fffffff)
                {
                    ...
                    {
                        /* Adjust the read count back to the correct value for
                         * this chunk.
                         */
                        file->read_count -= 8;
...
static void PNGCBAPI error_handler(png_structp png_ptr, png_const_charp message)
{
...
/* Return 'count' bytes to libpng in 'buffer' */
{
    ...
    /* The callback always reads ahead by 8 bytes - the signature or chunk
     * header
    ...
    if (file->read_count < 8)
    {
        ...
        if (file->state == STATE_SIGNATURE)
        {
            ...
            /* Else write it (this is the initialization of write_count, prior
             * to this it contains CLEAR garbage.)
             */
            file->write_count = 0;
        }
        ...
        else
        {
            ...
            /* The first chunk must be a well formed IHDR (this could be relaxed
             * to use the checks in process_chunk, but that seems unnecessary.)
             */
    ...
    {
        /* This is the signature case; for IDAT and other chunks these values
         * will be overwritten when read_chunk is called below.
         */
        length = file->length;
        type = file->type;
    }
    ...
    do
    {
         ...
         * a chunk control structure and sets the file length/type/data_pos
         * fields for the *NEXT* chunk header.
         *
         ...
         *
         * NOTE: read_chunk only returns when it has read a chunk that must now
         * be written.
         */
        ...
        case 8:
            ...
            if (file->state == STATE_SIGNATURE)
            {
                /* The signature has been written, the tail call to
                 * read_callback below (it's just a goto to the start with a
                 * decent compiler) will read the IHDR header ahead and validate
                 * it.
                 */
                ...
                if (file->state == STATE_IDAT)
                {
                    ...
                    while (idat->idat_index >= idat->idat_length)
                    {
                        ...
                        if (++(idat->idat_count) >= cur->count)
                        {
                            ...
                            /* This is an internal error - read beyond the end
                             * of the pre-calculated stream.
                             */
            ...
            case 11:
                ...
                if (file->state == STATE_IDAT
                    && (file->idat->idat_index < file->idat->idat_length
                        || 1 + file->idat->idat_count
                            < file->idat->idat_cur->count
 ...
 */
{
    ...
    if (png_ptr == NULL)
    {
        ...
        log_error(&control->file, LIBPNG_ERROR_CODE,
                  "OOM allocating png_struct");
...
static void usage(const char *prog)
{
    ...
    static const char *usage_string[] = {
        ...
        "OPTIONS",
        "  OPERATION",
        "      By default files are just checked for readability with a "
        "summary of the",
        ...
        "        unsafe: Remove chunks that may be unsafe to retain if the "
        "image data",
        ...
        "      Only ancillary chunks are ever removed.  In addition the tRNS "
        "and sBIT",
        ...
        "        gAMA, sRGB [all]: These specify the gamma encoding used for "
        "the pixel",
        "            values.",
        "        cHRM, iCCP [color]: These specify how colors are encoded.  "
        "iCCP also",
        "            specifies the exact encoding of a pixel value; however, "
        "in",
        ...
        "      chunks will be the maximum size permitted; 2^31-1 bytes.  If "
        "the option",
        "      is omitted the original chunk sizes will not be changed.  When "
        "the",
        "      option is given --strip=unsafe is set automatically. This may "
        "be",
        ...
        "    --quiet (-q):",
        "      Do not output the summaries except for files that cannot be "
        "read. With",
        "      two --quiets these are not output either.",
        "    --errors (-e):",
        "      Output errors from libpng and the program (except "
        "too-far-back).",
        "    --warnings (-w):",
...
#            endif
        ...
        "EXIT CODES",
        "  *** SUBJECT TO CHANGE ***",
        "  The program exit code is value in the range 0..127 holding a bit "
        "mask of",
        ...
        "  Errors less than 16 are potentially recoverable, for a single file "
        "if the",
        "  exit code is less than 16 the file could be read (with corrections "
        "if a",
        ...
        "exit",
        "  255.  Some older operating systems only support 7-bit exit codes, "
        "on those",
        ...
        "summary.",
        "  With the --quiet (-q) option the summaries are suppressed too and "
        "the",
        "  program only outputs unexpected errors (internal errors and file "
        "open",
        "  errors).",
        "  Various known problems in PNG files are fixed while the file is "
        "being read",
        "  The exit code says what problems were fixed.  In particular the "
        "zlib error:",
        0,
        "        \"invalid distance too far back\"",
        0,
        "  caused by an incorrect optimization of a zlib stream is fixed in "
        "any",
        "  compressed chunk in which it is encountered.  An integrity problem "
        "of the",
        ...
        "unless",
        "  either an invalid argument causes the usage message (this message) "
        "to be",
        "  produced or the program crashes.",
        0,
        "  The summary lines describe issues encountered with the zlib "
        "compressed",
        "  stream of a chunk.  They have the following format, which is "
        "SUBJECT TO",
        ...
        "     chunk ??? comp-level file-bits ok-bits compressed uncompress "
        "file",
        0,
        ...
        "          OPT: The zlib stream window bits value could be improved "
        "(and was).",
        "          SKP: The chunk was skipped because of a zlib issue "
        "(zlib-rc) with",
        "               explanation 'message'",
        "          ERR: The read of the file was aborted.  The parameters "
        "explain why.",
        "$3 status:     For 'ERR' the accumulated status code from 'EXIT "
        "CODES' above.",
        "               This is printed as a 2 digit hexadecimal value",
        "   comp-level: The recorded compression level (FLEVEL) of a zlib "
        "stream",
        "               expressed as a string "
        "{supfast,stdfast,default,maximum}",
        ...
        "the",
        "               reason is 'SKP'; this is a count of the bytes read "
        "from the",
        ...
        "this",
        "               may not be the same as the number of bytes in the "
        "image.",
