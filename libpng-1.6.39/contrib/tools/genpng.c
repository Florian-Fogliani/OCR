        ...
        for (dy = -FILTER_D; dy <= FILTER_D; ++dy)
        {
            ...
            if (wy != 0)
            {
                ...
                for (dx = -FILTER_D; dx <= FILTER_D; ++dx)
                {
                    ...
                    if (wx != 0
                        && arg->inside_fn(arg, x + dx / 16, y + dy / 16))
                        alphay += wx;
                }
...
static int check_line(const struct arg *arg, double x, double y)
{
                   ...
                   FILTER_WIDTH))
    {
        ...
        if (arg->width > 2 * FILTER_WIDTH
            && line_check(x, y, arg->x1, arg->y1, arg->x2, arg->y2,
                          arg->width / 2, -FILTER_WIDTH))
            return INSIDE;
...
#    define FN_CHECK 1
                   ...
                   { "circle",
                     { { inside_circle_filled, check_circle_filled },
                       { inside_circle, check_circle } } },
                   { "line",
                     { { NULL, NULL }, { inside_line, check_line } } } };
...
#    endif
        ...
        if (buffer != NULL)
        {
            ...
            for (y = 0; y < image.height; ++y)
            {
                ...
                for (x = 0; x < image.width; ++x)
                    pixel(buffer + 4 * (x + y * image.width), arg_list, nshapes,
                          x, y);
            }
    ...
    else
    {
        ...
        fprintf(
            ...
            "  y1,y2 ::= <number>\n"
            "\n"
            " Numbers are floating point numbers describing points relative "
            "to\n"
            " the top left of the output PNG as pixel coordinates.  The "
            "'width'\n"
            " parameter is either the width of the line (in output pixels) "
            "used\n"
            ...
            "\n"
            " Cyan is full intensity blue+green; RGB(0,1,1), plus the "
            "following\n"
            ...
            "\n"
            " The RGB values are selected to make detection of aliasing "
            "errors\n"
            " easy. The names are selected to make the description of errors\n"
            " easy.\n"
            "\n"
            " The PNG is written to stdout, if --8bit is given a 32bpp RGBA "
            "sRGB\n"
