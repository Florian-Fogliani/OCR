...
int main(int argc, char **argv)
{
    ...
    if (!inname)
    {
        ...
        {
            fprintf(stderr,
                    PROGNAME ":  must give input filename or provide image "
                             "data via stdin\n");
            ++error;
        }
        else
        {
            ...
            {
                fprintf(stderr,
                        PROGNAME ":  unable to reopen stdin in binary mode\n");
                ++error;
            }
            else if ((wpng_info.outfile = fdopen(fileno(stdout), "wb")) == NULL)
            {
                fprintf(stderr,
                        PROGNAME ":  unable to reopen stdout in binary mode\n");
    ...
    if ((rc = writepng_init(&wpng_info)) != 0)
    {
        ...
        case 2:
            fprintf(stderr,
                    PROGNAME ":  libpng initialization problem (longjmp)\n");
            break;
        case 4:
