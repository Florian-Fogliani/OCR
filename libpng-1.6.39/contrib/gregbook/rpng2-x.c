...
int main(int argc, char **argv)
{
    ...
    if (error)
    {
        ...
        fprintf(
            ...
            "    sec \tseconds to display each background image (default = "
            "2)\n");
#endif
        fprintf(
            stderr,
            ...
            "    -timing\tenables delay for every block read, to simulate "
            "modem\n"
            "\t\t  download of image (~36 Kbps)\n"
            "    -pause\tpauses after displaying each pass until mouse "
            "clicked\n"
            "\nPress Q, Esc or mouse button 1 (within image window, after "
            "image\n"
    ...
    else
    {
        ...
        else if ((rc = readpng2_init(&rpng2_info)) != 0)
        {
            ...
            case 2:
                fprintf(stderr,
                        PROGNAME ":  [%s] has bad IHDR (libpng longjmp)\n",
                        filename);
                break;
            case 4:
    ...
    for (;;)
    {
            ...
            if (error || incount != INBUFSIZE || rpng2_info.state == kDone)
        {
            ...
            {
                fprintf(stderr,
                        PROGNAME ":  error while reading PNG image file\n");
                exit(3);
            }
            else if (feof(infile))
            {
    ...
    if (loop && bg_image)
    {
                        ...
                        && now.tv_usec < then.tv_usec))
                {
                    ...
                    while (seconds_to_go-- > 1)
                    {
                        ...
                        for (i = seconds_to_go; i > 0 && !quit; --i)
                        {
                            ...
                            if (XCheckMaskEvent(
                                    display, KeyPressMask | ButtonPressMask, &e)
                                && QUIT(e, k))
                                quit = TRUE;
        ...
        if (rpng2_info.state >= kWindowInit)
        {
            Trace((stderr, "entering final wait-for-quit-event loop\n")) do
            {
                ...
                if (e.type == Expose)
                {
                    ...
                    rpng2_x_redisplay_image(ex->x, ex->y, ex->width,
                                            ex->height);
                }
            }
            while (!QUIT(e, k))
        ...
        {
            fprintf(stderr,
                    PROGNAME ":  init callback never called:  probable "
...
static int rpng2_x_create_window(void)
{
    ...
    if (depth != 16 && depth != 24 && depth != 32)
    {
        ...
        {
            /* GRR:  add 15-, 16- and 32-bit TrueColor visuals (also
             * DirectColor?) */
            fprintf(stderr,
    ...
    window =
        XCreateWindow(display, root, 0, 0, rpng2_info.width, rpng2_info.height,
                      0, depth, InputOutput, visual, attrmask, &attr);
...
static int rpng2_x_load_bg_image(void)
{
    ...
    if ((bg[pat].type & 0x07) == 0)
    {
        ...
        /*---------------------------------------------------------------------------
            Soft gradient-diamonds with scale = bgscale.  Code contributed by
          Adam M. Costello.
          ---------------------------------------------------------------------------*/
    }
    else if ((bg[pat].type & 0x07) == 1)
    {
        ...
        /*---------------------------------------------------------------------------
            Radial "starburst" with azimuthal sinusoids; [eventually number of
          sinu- soids will equal bgscale?].  This one is slow but very cool.
          Code con- tributed by Pieter S. van der Meulen (originally in
          Smalltalk).
          ---------------------------------------------------------------------------*/
    }
    else if ((bg[pat].type & 0x07) == 2)
    {
        ...
        for (row = 0; row < rpng2_info.height; ++row)
        {
            ...
            for (i = 0; i < rpng2_info.width; ++i)
            {
                ...
                intensity = cos((angle + (rotate * dist * PI)) * freq) * gray
                    * saturate;
        ...
        for (row = 0; row < rpng2_info.height; ++row)
        {
            ...
            { /* slightly optimized version */
                for (i = rpng2_info.width; i > 0; --i)
                {
                    ...
                    blue = *src++;
                    pixel =
                        (red << RShift) | (green << GShift) | (blue << BShift);
                                    ...
                                    ulg height)
{
    ...
    Trace((stderr, "beginning display loop (image_channels == %d)\n",
        ...
        /*---------------------------------------------------------------------------
          ...
          image is fully decoded and therefore is, by definition,
          non-progressive.
          ---------------------------------------------------------------------------*/
