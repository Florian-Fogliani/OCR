        ...
        do
        {
            ...
            do
            {
                ...
                if (in_alpha > 0)
                {
                    ...
                    if (in_alpha > 0)
                    {
                        /* The input must be composed onto the output. This
                         * means multiplying the current output pixel value by
                         * the inverse of the input alpha (1-alpha). A division
                         * is required but it is by the constant 65535.
                         * Approximate this as:
                         *
                         *     (x + (x >> 16) + 32769) >> 16;
                         *
                         * This is exact (and does not overflow) for all values
                         * of x in the range 0..65535*65535.  (Note that the
                         * calculation produces the closest integer; the maximum
                         * error is <0.5).
                         */
                        png_uint_32 tmp;
...
static int create_sprite(struct sprite *sprite, int *argc, const char ***argv)
{
            ...
            if (png_image_begin_read_from_file(&image, (*argv)[0]))
            {
                ...
                if (buffer != NULL)
                {
                    if (png_image_finish_read(
                            &image, NULL /*background*/, buffer,
                            0 /*row_stride*/,
                            NULL /*colormap for PNG_FORMAT_FLAG_COLORMAP*/))
                    {
                        /* This is the place where the Porter-Duff 'Over'
                         * operator needs to be done by this code.  In fact, any
                         * image processing required can be done here; the data
                         * is in the correct format (linear, 16-bit) and source
                         * and destination are in memory.
                         */
                ...
                else
                {
                    ...
                    /* png_image_free must be called if we abort the Simplified
                     * API read because of a problem detected in this code.  If
                     * problems are detected in the Simplified API it cleans up
                     * itself.
                     */
    ...
    if (sprite->file != NULL)
    {
        ...
        if (png_image_write_to_stdio(&save, sprite->file,
                                     1 /*convert_to_8_bit*/, sprite->buffer,
    ...
    while (argc > 0)
    {
            ...
            if (nsprites < csprites)
            {
                    ...
                    && sprites[nsprites].height > 0)
                {
                    ...
                    if (sprites[nsprites].name[0] == 0)
                        sprintf(sprites[nsprites].name, "sprite-%d",
                                nsprites + 1);
                    ...
                    if (tmp == sizeof(png_uint_16[4]))
                    {
                        ...
                        if (sprites[nsprites].buffer != NULL)
                        {
                            ...
                            if (create_sprite(sprites + nsprites++, &argc,
                                              &argv))
                            {
                                result = 1; /* still ok */
                                continue;
                ...
                else
                {
                    fprintf(stderr, "simpleover: %s: invalid sprite (%u,%u)\n",
                            argv[0], sprites[nsprites].width,
                            sprites[nsprites].height);
                    break;
                }
            }
        ...
        else if (strncmp(argv[0], "--add=", 6) == 0)
        {
            ...
            while (--isprite >= 0)
            {
                ...
                {
                    if (!add_sprite(output, out_buf, sprites + isprite, &argc,
                                    &argv))
...
int main(int argc, const char **argv)
{
    ...
    if (argc >= 2)
    {
        ...
        if (png_image_begin_read_from_file(&image, argv[1]))
        {
            ...
            if (buffer != NULL)
            {
                png_color background = { 0, 0xff,
                                         0 }; /* fully saturated green */
                        ...
                        NULL /*colormap for PNG_FORMAT_FLAG_COLORMAP */))
                {
                     ...
                     * allocated data in png_image, and only the buffer needs to
                     * be freed.
                     *
                     * Perform the remaining operations:
                     */
                    if (simpleover_process(&image, buffer, argc - argi,
                                           argv + argi))
                    {
                        /* Write the output: */
                        if ((output != NULL
                            ...
                            || (output == NULL
                                && png_image_write_to_stdio(
                                    &image, stdout, 0 /*convert_to_8bit*/,
                                    buffer, 0 /*row_stride*/,
                                    NULL /*colormap*/)))
                            result = 0;
            ...
            else
            {
                ...
                /* This is the only place where a 'free' is required; libpng
                 * does the cleanup on error and success, but in this case we
                 * couldn't complete the read because of running out of memory.
                 */
                png_image_free(&image);
            }
        }
    ...
    else
    {
        ...
        fprintf(
            ...
            "  Output 'background.png' as a 24-bit RGB PNG file in "
            "'output.png'\n"
            "   or, if not given, stdout.  'background.png' will be "
            "composited\n"
            ...
            "     operation at offset (x,y) in the sprite (defaulting to "
            "(0,0)).\n"
            ...
            "  line and thus the first PNG processed appears as the "
            "bottommost\n"
            "  in the output image.\n");
    }
