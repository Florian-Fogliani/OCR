...
#    define png_libpng_ver png_get_header_ver(NULL)
    ...
    /* Basic control structions.  Read libpng-manual.txt or libpng.3 for more
     * info.
     *
     * png_struct is the cache of information used while reading or writing a
     * single PNG file.  One of these is always required, although the
     * simplified API (below) hides the creation and destruction of it.
     */
     ...
     * One or more of these must exist while reading or creating a PNG file. The
    ...
    /* Types with names ending 'p' are pointer types.  The corresponding types
     * with names ending 'rp' are identical pointer types except that the
     * pointer is marked 'restrict', which means that it is the only pointer to
     * the object passed to the function.  Applications should not use the
     * 'restrict' types; it is always valid to pass 'p' to a pointer with a
     * function argument of the corresponding 'rp' type.  Different compilers
     * have different rules with regard to type matching in the presence of
     * 'restrict'.  For backward compatibility libpng callbacks never have
     * 'restrict' in their parameters and, consequentially, writing portable
     * application code is extremely difficult if an attempt is made to use
     * 'restrict'.
     */
    ...
    /*  When the depth of the sPLT palette is 8 bits, the color and alpha
     * samples occupy the LSB of their respective members, and the MSB of each
     * member is zero-filled.  The frequency member always occupies the full 16
     * bits.
     */
...
#    ifdef PNG_TEXT_SUPPORTED
     ...
     * other string-handling functions.  Note that the "itxt_length", "lang",
     * and "lang_key" members of the structure only exist when the library is
     * built with iTXt chunk support.  Prior to libpng-1.4.0 the library was
     * built by default without iTXt support. Also note that when iTXt *is*
     * supported, the "lang" and "lang_key" fields contain NULL pointers when
     * the "compression" field contains * PNG_TEXT_COMPRESSION_NONE or
...
#    if defined(PNG_STORE_UNKNOWN_CHUNKS_SUPPORTED)                            \
    ...
    typedef struct png_unknown_chunk_t
    {
         ...
         * Notice that on read it is set by libpng however the values stored
         * have more bits set than are listed below.  Always treat the value as
         * a bitmask.  On write set only one bit - setting multiple bits may
         * cause the chunk to be written in multiple places.
         */
...
#    define PNG_INFO_eXIf 0x10000U /* GR-P, 1.6.31 */
     ...
     * modify the buffer it is passed. The 'read' function, on the other hand,
     * is expected to return the read data in the buffer.
     */
...
#    ifdef PNG_PROGRESSIVE_READ_SUPPORTED
     ...
     * row number is the row number within the sub-image of the interlace pass,
     * so the value will increase to the height of the sub-image (not the full
     * image) then reset to 0 for the next pass.
     *
     * Use PNG_ROW_FROM_PASS_ROW(row, pass) and PNG_COL_FROM_PASS_COL(col, pass)
...
#    ifdef PNG_SETJMP_SUPPORTED
     ...
     * application must include this before png.h to obtain the definition of
     * jmp_buf.  The function is required to be PNG_NORETURN, but this is not
     * checked.  If the function does return the application will crash via an
     * abort() or similar system level call.
     *
     * If you get a warning here while building the library you may need to make
     * changes to ensure that pnglibconf.h records the calling convention used
     * by your compiler.  This may be very difficult - try using a different
     * compiler to build the library!
     */
...
#    ifdef PNG_READ_SUPPORTED
    ...
    PNG_EXPORTA(10, int, png_reset_zstream, (png_structrp png_ptr),
                PNG_DEPRECATED);
#    endif
...
#    endif
     ...
     * default allocation method (typically malloc).  Use is deprecated in 1.6.0
     * and the API will be removed in the future.
     */
...
#        if PNG_LIBPNG_VER < 10700
    /* To do: remove this from libpng17 (and from libpng17/png.c and
     * pngstruct.h) */
...
#    ifdef PNG_READ_EXPAND_16_SUPPORTED
     ...
     * expansion of a tRNS chunk if present.
     */
...
#        define PNG_RGB_TO_GRAY_DEFAULT (-1) /*for red/green coefficients*/
    ...
    PNG_FIXED_EXPORT(33, void, png_set_rgb_to_gray_fixed,
                     (png_structrp png_ptr, int error_action,
...
#        define PNG_ALPHA_ASSOCIATED                                           \
            1 /* as above; this is the normal practice */
#        define PNG_ALPHA_PREMULTIPLIED 1 /* as above */
#        define PNG_ALPHA_OPTIMIZED                                            \
            2 /* 'PNG' for opaque pixels, else 'STANDARD' */
...
#        define PNG_GAMMA_sRGB                                                 \
            220000 /* Television standards--matches sRGB gamma */
...
#    endif
     ...
     * shortcoming of assuming that input PNG data with no gamma information is
     * linear - this is unlikely to be correct unless the PNG files where
     * generated locally. Most of the time the output precision will be so low
     * as to show significant banding in dark areas of the image.
     *
     ...
     * accurate scaling and processing of the data.  If you know that your input
     * PNG files were generated locally you might need to replace
     ...
     * (you just skip those) you do the composition math using png_composite or
     ...
     * because of the software or hardware you use then you have a big problem.
     * The PNG case will probably result in halos around the image.  The linear
     * encoding will probably result in a washed out, too bright, image (it's
     * actually too contrasty.)  Try the ALPHA_OPTIMIZED mode above - this will
     * probably substantially reduce the halos.  Alternatively try:
     *
     ...
     * allows you to provide a default gamma, but it also sets the output gamma
     * to the matching value.  If you know your PNG files have a gamma that
...
#    ifdef PNG_READ_BACKGROUND_SUPPORTED
     ...
     * been read.  Doing so will result in unexpected behavior and possible
     * warnings or errors if the PNG file contains a bKGD chunk.
     */
...
#        define PNG_GAMMA_THRESHOLD (PNG_GAMMA_THRESHOLD_FIXED * .00001)
     ...
     * therefore override the value for gamma in a PNG file if it is called
     * after the file header has been read - use with care  - call before
     * reading the PNG file for best results!
     *
     * These routines accept the same gamma values as png_set_alpha_mode
     * (described above).  The PNG_GAMMA_ defines and PNG_DEFAULT_sRGB can be
     * passed to either API (floating point or fixed.)  Notice, however, that
     * the 'file_gamma' value is the inverse of a 'screen gamma' value.
     */
...
#    endif
     ...
     * of libpng and to allow the 'display_row' array from read_rows to be
     * passed unchanged to write_rows.
     */
...
#    endif
    ...
    PNG_EXPORT(77, void, png_set_write_fn,
               (png_structrp png_ptr, png_voidp io_ptr,
...
#    ifdef PNG_USER_MEM_SUPPORTED
    /* Replace the default memory allocation functions with user supplied
     * one(s). */
    PNG_EXPORT(82, void, png_set_mem_fn,
               (png_structrp png_ptr, png_voidp mem_ptr,
...
#    ifdef PNG_USER_TRANSFORM_INFO_SUPPORTED
    /* Return information about the row currently being processed.  Note that
     * these APIs do not fail but will return unexpected results if called
     * outside a user transform callback.  Also note that when transforming an
     * interlaced image the row number is the row number within the sub-image of
     * the interlace pass, so the value will increase to the height of the
     * sub-image (not the full image) then reset to 0 for the next pass.
     *
     * Use PNG_ROW_FROM_PASS_ROW(row, pass) and PNG_COL_FROM_PASS_COL(col, pass)
     * to find the output pixel (x,y) given an interlaced sub-image pixel
     * (row,col,pass).  (See below for these macros.)
...
#    ifdef PNG_READ_USER_CHUNKS_SUPPORTED
     ...
     * chunks to be treated as unknown, however in this case the callback must
     * do any processing required by the chunk (e.g. by calling the appropriate
     * png_set_ APIs.)
     *
     ...
     *           chunk will cause an error at this point unless it is to be
     * saved. positive: The chunk was handled, libpng will ignore/discard it.
     *
     * See "INTERACTION WITH USER CHUNK CALLBACKS" below for important notes
     * about how this behavior will change in libpng 1.7
     */
...
#    ifdef PNG_PROGRESSIVE_READ_SUPPORTED
     ...
     * 'save' is set to true the routine will first save all the pending data
...
#    endif /* PROGRESSIVE_READ */
     ...
     * passed in, it does not change the state for other png_info structures.
     *
     * It is unlikely that this function works correctly as of 1.6.0 and using
     * it may result either in memory leaks or double free of allocated data.
     */
...
#    endif
    ...
    /* The png_set_<chunk> functions are for storing values in the
     * png_info_struct. Similarly, the png_get_<chunk> calls are used to read
     * values from the png_info_struct, either storing the parameters in the
     * passed variables, or setting pointers into the png_info_struct where the
     * data is stored.  The png_get_<chunk> functions return a non-zero value if
     * the data was available in info_ptr, or return zero and do not change any
     * of the parameters if the data was not available.
     *
...
#        if defined(PNG_FLOATING_ARITHMETIC_SUPPORTED)                         \
     ...
     * it is highly recommended that png_get_sCAL_s be used instead.
     */
...
#        endif /* HANDLE_AS_UNKNOWN */
    ...
    /* The "keep" PNG_HANDLE_CHUNK_ parameter for the specified chunk is
     * returned; the result is therefore true (non-zero) if special handling is
     * required, false for the default handling.
     */
...
#    ifdef PNG_IO_STATE_SUPPORTED
    ...
    PNG_REMOVED(200, png_const_bytep, png_get_io_chunk_name,
                (png_structrp png_ptr), PNG_DEPRECATED)
...
#    ifdef PNG_READ_COMPOSITE_NODIV_SUPPORTED
     ...
     * most machines.  However, it does take more operations than the
     * corresponding divide method, so it may be slower on a few RISC systems.
     * There are two shifts (by 8 or 16 bits) and an addition, versus a single
     * integer divide.
     *
            ...
            {                                                                  \
                png_uint_32 temp =                                             \
                    (png_uint_32)((png_uint_32)(fg) * (png_uint_32)(alpha)     \
                                  + (png_uint_32)(bg)                          \
                                      * (65535 - (png_uint_32)(alpha))         \
                                  + 32768);                                    \
                (composite) =                                                  \
                    (png_uint_16)(0xffff & ((temp + (temp >> 16)) >> 16));     \
            }
...
#        define PNG_IMAGE_VERSION 1
    ...
    typedef struct
    {
        ...
        /* In the event of an error or warning the following field will be set
         * to a non-zero value and the 'message' field will contain a '\0'
         ...
         *
         * The upper 30 bits of this value are reserved, the low two bits
         * contain a value as follows:
         */
...
#            define PNG_FORMAT_FLAG_BGR                                        \
                0x10U /* BGR colors, else order is RGB */
#        endif
...
#        define PNG_IMAGE_ROW_STRIDE(image)                                    \
     ...
     * row.
     *
     * WARNING: this macro overflows for some images with more than one
     * component and very large image widths.  libpng will refuse to process an
     * image where this macro would overflow.
     */
...
#        define PNG_IMAGE_BUFFER_SIZE(image, row_stride)                       \
     ...
     * libpng will refuse to process an image where such an overflow would
     * occur.
     */
...
#        define PNG_IMAGE_FLAG_FAST 0x02
     ...
     * only used it when producing intermediate files that will be read back in
...
#        define PNG_IMAGE_FLAG_16BIT_sRGB 0x04
     ...
     * setting this flag only affects the interpretation of 16-bit images from
     * an external source.  It is recommended that the application expose this
     * flag to the user; the user can normally easily recognize the difference
     * between linear and sRGB encoding.  This flag has no effect on write - the
     * data passed to the write APIs must have the correct encoding (as defined
     * above.)
     *
...
#            endif /* STDIO */
     ...
     *
     * For linear output removing the alpha channel is always done by
     * compositing on black and background is ignored.
     *
    ...
    /* Free any data allocated by libpng in image->opaque, setting the pointer
     * to NULL.  May be called at any time after the structure is initialized.
     */
...
#            endif /* SIMPLIFIED_WRITE_STDIO */
     ...
     * data then setting convert_to_8_bit will cause the output to be an 8-bit
     * PNG gamma encoded according to the sRGB specification, otherwise a 16-bit
     * linear encoded PNG file is written.
     *
     * With color-mapped data formats the colormap parameter point to a
     * color-map with at least image->colormap_entries encoded in the specified
     * format.  If the format is linear the written PNG color-map will be
     * converted to sRGB regardless of the convert_to_8_bit flag.
     *
     * With all APIs row_stride is handled as in the read APIs - it is the
     * spacing from one row to the next in component sized units (1 or 2 bytes)
     * and if negative indicates a bottom-up row layout in the buffer.  If
     * row_stride is zero, libpng will calculate it for you from the image width
     * and number of channels.
     *
     * Note that the write API does not support interlacing, sub-8-bit pixels or
     * most ancillary chunks.  If you need to write text chunks (e.g. for
     * copyright notices) you need to use one of the other APIs.
     */
...
#            define png_image_write_get_memory_size(                           \
                image, size, convert_to_8_bit, buffer, row_stride, colormap)   \
                png_image_write_to_memory(&(image), 0, &(size),                \
                                          convert_to_8_bit, buffer,            \
                                          row_stride, colormap)
 ...
 */
#            define PNG_IMAGE_DATA_SIZE(image)                                 \
     ...
     * height.  The same goes for the remainder of these macros; they all
     * produce bigger numbers than the actual in-memory image size.
     */
...
#            define PNG_IMAGE_PNG_SIZE_MAX_(image, image_size)                 \
                ((8U /*sig*/ + 25U /*IHDR*/ + 16U /*gAMA*/ + 44U /*cHRM*/      \
                  + 12U /*IEND*/                                               \
                  + (((image).format & PNG_FORMAT_FLAG_COLORMAP)               \
                         ...
                         : 0U)                                                 \
                  + 12U)                                                       \
                 + (12U * ((image_size) / PNG_ZBUF_SIZE)) /*IDAT*/             \
                 + (image_size))
...
#            define PNG_IMAGE_PNG_SIZE_MAX(image)                              \
                PNG_IMAGE_PNG_SIZE_MAX_(image,                                 \
                                        PNG_IMAGE_COMPRESSED_SIZE_MAX(image))
    /* An upper bound on the total length of the PNG data stream for 'image'.
     * The result is of type png_alloc_size_t, on 32-bit systems this may
     * overflow even though PNG_IMAGE_DATA_SIZE does not overflow; the write
     * will run out of buffer space but return a corrected size which should
     * work.
     */
...
#            define PNG_ARM_NEON                                               \
                0 /* HARDWARE: ARM Neon SIMD instructions supported */
#        endif
#        define PNG_MAXIMUM_INFLATE_WINDOW                                     \
            2 /* SOFTWARE: force maximum window */
#        define PNG_SKIP_sRGB_CHECK_PROFILE                                    \
            4 /* SOFTWARE: Check ICC profile for sRGB */
#        ifdef PNG_MIPS_MSA_API_SUPPORTED
#            define PNG_MIPS_MSA                                               \
                6 /* HARDWARE: MIPS Msa SIMD instructions supported */
#        endif
