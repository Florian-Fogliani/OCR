...
#    define PNG_INTERNAL_FUNCTION(type, name, args, attributes)                \
        PNG_LINKAGE_FUNCTION PNG_FUNCTION(type, name, args,                    \
 ...
 * <fp.h> if possible.
 */
#            if !defined(__MATH_H__) && !defined(__MATH_H)                     \
                && !defined(__cmath__)
#                include <fp.h>
#            endif
#        else
...
#    define png_isaligned(ptr, type)                                           \
        (((type)(size_t)((const void *)(ptr)) & (type)(png_alignof(type) - 1)) \
         == 0)
#else
...
#    define PNG_UNEXPECTED_ZLIB_RETURN (-7)
    PNG_INTERNAL_FUNCTION(void, png_zstream_error,
                          (png_structrp png_ptr, int ret), PNG_EMPTY);
    /* Used by the zlib handling functions to ensure that z_stream::msg is
     * always set before they return.
     */
...
#    endif
    ...
    /* Check the user version string for compatibility, returns false if the
     * version numbers aren't compatible.
     */
     ...
     * does, however, call the application provided allocator and that could
     * call png_error (although that would be a bug in the application
     * implementation.)
     */
...
#    if defined(PNG_TEXT_SUPPORTED) || defined(PNG_sPLT_SUPPORTED)             \
    ...
    /* Internal array allocator, outputs no error or warning messages on
     * failure, just returns NULL.
     */
    ...
    /* The same but an existing array is extended by add_elements.  This
     * function also memsets the new elements to 0 and copies the old elements.
     * The old array is not freed or altered.
     */
...
#    endif /* text, sPLT or unknown chunks */
    ...
    /* Magic to create a struct when there is no struct to call the user
     * supplied memory allocators.  Because error handling has not been set up
     * the memory handlers can't safely call png_error, but this is an obscure
     * and undocumented restriction so libpng has to assume that the 'free'
     * handler, at least, might call png_error.
     */
    ...
    PNG_INTERNAL_FUNCTION(void, png_free_jmpbuf, (png_structrp png_ptr),
                          PNG_EMPTY);
    ...
    PNG_INTERNAL_FUNCTION(void, png_zfree, (voidpf png_ptr, voidpf ptr),
                          PNG_EMPTY);

    /* Next four functions are used internally as callbacks.  PNGCBAPI is
     * required but not PNG_EXPORT.  PNGAPI added at libpng version 1.2.3,
     * changed to PNGCBAPI at 1.5.0
     */
...
#    endif
    ...
    PNG_INTERNAL_FUNCTION(void, png_crc_read,
                          (png_structrp png_ptr, png_bytep buf,
                           png_uint_32 length),
                          PNG_EMPTY);
    ...
    PNG_INTERNAL_FUNCTION(int, png_crc_error, (png_structrp png_ptr),
                          PNG_EMPTY);
...
#    endif
    ...
    PNG_INTERNAL_FUNCTION(void, png_write_IEND, (png_structrp png_ptr),
                          PNG_EMPTY);
...
#    ifdef PNG_WRITE_sRGB_SUPPORTED
    PNG_INTERNAL_FUNCTION(void, png_write_sRGB,
                          (png_structrp png_ptr, int intent), PNG_EMPTY);
#    endif
...
#    ifdef PNG_WRITE_tRNS_SUPPORTED
    PNG_INTERNAL_FUNCTION(void, png_write_tRNS,
                          (png_structrp png_ptr, png_const_bytep trans,
                           png_const_color_16p values, int number,
                           int color_type),
                          PNG_EMPTY);
#    endif
...
#    ifdef PNG_WRITE_pCAL_SUPPORTED
    PNG_INTERNAL_FUNCTION(void, png_write_pCAL,
                          (png_structrp png_ptr, png_charp purpose,
...
#    endif
    PNG_INTERNAL_FUNCTION(void, png_combine_row,
                          (png_const_structrp png_ptr, png_bytep row,
                           int display),
                          PNG_EMPTY);
...
#    ifdef PNG_READ_INTERLACING_SUPPORTED
     ...
     * expanded (moved apart) in 'row' to match the final layout, when doing
     * this the pixels are *replicated* to the intervening space.  This is
     * essential for the correct operation of png_combine_row, above.
     */
...
#    endif
    ...
    /* Unfilter a row: check the filter value before calling this, there is no
     * point calling it for PNG_FILTER_VALUE_NONE.
     */
...
#    ifdef PNG_SEQUENTIAL_READ_SUPPORTED
    PNG_INTERNAL_FUNCTION(void, png_read_IDAT_data,
                          ...
                          PNG_EMPTY);
    /* Read 'avail_out' bytes of data from the IDAT stream.  If the output
     * buffer is NULL the function checks, instead, for the end of the stream.
     * In this case a benign error will be issued if the stream end is not found
     * or if extra data has to be consumed.
     */
...
#    if ZLIB_VERNUM >= 0x1240
    PNG_INTERNAL_FUNCTION(int, png_zlib_inflate,
                          (png_structrp png_ptr, int flush), PNG_EMPTY);
...
#        if defined(PNG_READ_SWAP_SUPPORTED)                                   \
            || defined(PNG_WRITE_SWAP_SUPPORTED)
...
#    ifdef PNG_COLORSPACE_SUPPORTED
    /* These internal functions are for maintaining the colorspace structure
     * within a png_info or png_struct (or, indeed, both).
     */
    ...
    PNG_INTERNAL_FUNCTION(int, png_colorspace_set_endpoints,
                          (png_const_structrp png_ptr,
                           png_colorspacerp colorspace, const png_XYZ *XYZ,
                           int preferred),
                          PNG_EMPTY);
...
#        ifdef PNG_sRGB_SUPPORTED
    PNG_INTERNAL_FUNCTION(int, png_colorspace_set_sRGB,
                          (png_const_structrp png_ptr,
                           png_colorspacerp colorspace, int intent),
                          PNG_EMPTY);
    /* This does set the colorspace gAMA and cHRM values too, but doesn't set
     * the flags to write them, if it returns false there was a problem and an
     * error message has already been output (but the colorspace may still need
     * to be synced to record the invalid flag).
     */
#        endif /* sRGB */
...
#            ifdef PNG_sRGB_SUPPORTED
    PNG_INTERNAL_FUNCTION(void, png_icc_set_sRGB,
                          (png_const_structrp png_ptr,
                           png_colorspacerp colorspace, png_const_bytep profile,
                           uLong adler),
                          PNG_EMPTY);
...
#    endif
    ...
    /* Puts 'string' into 'buffer' at buffer[pos], taking care never to
     * overwrite the end.  Always leaves the buffer nul terminated.  Never
     * errors out (and there is no error code.)
     */
...
#    if defined(PNG_WARNINGS_SUPPORTED) || defined(PNG_TIME_RFC1123_SUPPORTED)
    /* Utility to dump an unsigned value into a buffer, given a start pointer
     * and and end pointer (which should point just *beyond* the end of the
     * buffer!) Returns the pointer to the start of the formatted string.  This
     * utility only does unsigned values.
     */
...
#    ifdef PNG_BENIGN_ERRORS_SUPPORTED
     ...
     * 'error' for one which can be handled safely but which may lose
     * application information or settings.
     *
     * By default these both result in a png_error call prior to release, while
     * in a released version the 'warning' is just a warning.  However if the
     * application explicitly disables benign errors (explicitly permitting the
     * code to lose information) they both turn into warnings.
     *
     * If benign errors aren't supported they end up as the corresponding base
     * call (png_warning or png_error.)
     */
...
#        define PNG_FP_WAS_VALID                                               \
            64 /* Preceding substring is a valid fp number */
...
#        define PNG_FP_INVALID                                                 \
            512 /* Available for callers as a distinct value */
...
#        define PNG_FP_NZ_MASK                                                 \
            (PNG_FP_SAW_DIGIT | PNG_FP_NEGATIVE | PNG_FP_NONZERO)
...
#        endif
     ...
     * errors that might occur.  Returns true on success, false on failure
     * (either of the function or as a result of a png_error.)
     */
...
#    if PNG_ARM_NEON_IMPLEMENTATION == 1
    PNG_INTERNAL_FUNCTION(void, png_riffle_palette_neon, (png_structrp),
                          PNG_EMPTY);
