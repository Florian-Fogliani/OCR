...
static void write_unknown_chunks(png_structrp png_ptr,
                                 png_const_inforp info_ptr, unsigned int where)
{
...
#        ifdef PNG_SET_UNKNOWN_CHUNKS_SUPPORTED
                ...
                /* NOTE: this code is radically different from the read side in
                 * the matter of handling an ancillary unknown chunk.  In the
                 * read side the default behavior is to discard it, in the code
                 * below the default behavior is to write it.  Critical chunks
                 * are, however, only written if explicitly listed or if the
                 * default is set to write all unknown chunks.
                 *
                 * The default handling is also slightly weird - it is not
                 * possible to stop the writing of all unsafe-to-copy chunks!
                 *
...
#    endif
         ...
         *
         * 1.6.0: COLORSPACE support controls the writing of these chunks too,
         * and the chunks will be written if the WRITE routine is there and
...
#    ifdef PNG_COLORSPACE_SUPPORTED
            /* Write only one of sRGB or an ICC profile.  If a profile was
             * supplied and it matches one of the known sRGB ones issue a
             * warning.
             */
...
#            ifdef PNG_WRITE_sRGB_SUPPORTED
            if ((info_ptr->valid & PNG_INFO_sRGB) != 0)
                png_app_warning(
                    png_ptr, "profile matches sRGB but writing iCCP instead");
#            endif
...
#    endif /* USER_MEM */
    if (png_ptr != NULL)
    {
         ...
         * the application after the struct has been created.
         */
        png_ptr->zbuffer_size = PNG_ZBUF_SIZE;

        /* The 'zlib_strategy' setting is irrelevant because png_default_claim
         * in pngwutil.c defaults it according to whether or not filters will be
...
#    endif
        ...
        /* App warnings are warnings in release (or release candidate) builds
         * but are errors during development.
         */
...
#    endif
    ...
    /* At this point the row_info pixel depth must match the 'transformed'
     * depth, which is also the output depth.
     */
...
#    endif
     ...
     * point: the jmp_buf may still have to be freed.  See
     * png_destroy_png_struct for how this happens.
     */
}
...
#    ifdef PNG_WRITE_FILTER_SUPPORTED
         ...
         *
         * NOTE: this is a nasty constraint on the code, because it means that
         * the prev_row buffer must be maintained even if there are currently no
        ...
        if (png_ptr->row_buf != NULL)
        {
             ...
             * images cannot benefit from certain filters.  If this isn't done
             * here the check below will fire on 1 pixel high images.
             */
            if (png_ptr->height == 1)
            ...
            if ((filters & (PNG_FILTER_UP | PNG_FILTER_AVG | PNG_FILTER_PAETH))
                ...
                && png_ptr->prev_row == NULL)
            {
                /* This is the error case, however it is benign - the previous
                 * row is not available so the filter can't be used.  Just warn
                 * here.
                 */
                png_app_warning(
                    png_ptr,
...
#        endif
    ...
    for (; y > 0; --y)
    {
        ...
        png_write_row(png_ptr,
                      png_voidcast(png_const_bytep, display->local_row));
                  ...
                  png_uint_32 reciprocal /*from the above macro*/)
{
    ...
    else if (component > 0)
    {
         ...
         * value that becomes 255 is 65407. NOTE: this must agree with the
         * PNG_DIV257 macro (which must, therefore, be exact!)  [Could also test
         * reciprocal != 0]
         */
        if (alpha < 65407)
        {
...
#        endif
        ...
        for (; y > 0; --y)
        {
            ...
            while (out_ptr < row_end)
            {
                ...
                do /* always at least one channel */
                    *out_ptr++ =
                        png_unpremultiply(*in_ptr++, alpha, reciprocal);
                while (--c > 0);
            ...
            png_write_row(png_ptr,
                          png_voidcast(png_const_bytep, display->local_row));
            input_row +=
                (png_uint_16)display->row_bytes / (sizeof(png_uint_16));
        } /* while y */
    }
        ...
        if ((format & PNG_FORMAT_FLAG_LINEAR) != 0)
        {
            ...
            if ((channels & 1) != 0) /* no alpha */
            {
                if (channels >= 3) /* RGB */
                {
                    ...
                    palette[i].green =
                        (png_byte)PNG_sRGB_FROM_LINEAR(255 * entry[1]);
                    palette[i].red =
                        (png_byte)PNG_sRGB_FROM_LINEAR(255 * entry[bgr]);
                }
            ...
            else /* alpha */
            {
                 ...
                 * above this is designed to produce a value scaled to 255*65535
                 * when divided by 128 (i.e. asr 7).
                 */
                if (alphabyte > 0 && alphabyte < 255)
                    reciprocal =
                        (((0xffff * 0xff) << 7) + (alpha >> 1)) / alpha;
                ...
                if (channels >= 3) /* RGB */
                {
                    palette[i].blue = png_unpremultiply(
                        entry[afirst + (2 ^ bgr)], alpha, reciprocal);
                    palette[i].green =
                        png_unpremultiply(entry[afirst + 1], alpha, reciprocal);
                    palette[i].red = png_unpremultiply(entry[afirst + bgr],
                                                       alpha, reciprocal);
                }
...
#        endif
    ...
    if ((format
         & ~(png_uint_32)(PNG_FORMAT_FLAG_COLOR | PNG_FORMAT_FLAG_LINEAR
                          | PNG_FORMAT_FLAG_ALPHA | PNG_FORMAT_FLAG_COLORMAP))
        != 0)
...
static void(PNGCBAPI image_memory_write)(png_structp png_ptr,
                                         png_bytep /*const*/ data, size_t size)
{
    png_image_write_control *display =
    ...
    if (image != NULL && image->version == PNG_IMAGE_VERSION)
    {
        if (memory_bytes != NULL && buffer != NULL)
        {
            ...
            if (png_image_write_init(image) != 0)
            {
                ...
                result =
                    png_safe_execute(image, png_image_write_memory, &display);
            ...
            return png_image_error(
                image, "png_image_write_to_memory: invalid argument");
    }
                                    ...
                                    png_int_32 row_stride, const void *colormap)
{
    ...
    if (image != NULL && image->version == PNG_IMAGE_VERSION)
    {
        if (file != NULL && buffer != NULL)
        {
            ...
            return png_image_error(
                image, "png_image_write_to_stdio: invalid argument");
    }
